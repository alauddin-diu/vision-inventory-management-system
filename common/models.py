...
class MetaMixin(models.Model):
    """A base class for Vision_IMS models to include shared meta fields.
...
class BaseURLValidator(URLValidator):
    """Validator for the Vision_IMS base URL:
...
class ProjectCode(Vision_IMS.models.MetadataMixin, models.Model):
...
class BaseVision_IMSSetting(models.Model):
    """An base Vision_IMSSetting object is a key:value pair used for storing single values (e.g. one-off settings values).
    ...
    class Meta:
        """Meta options for BaseVision_IMSSetting -> abstract stops creation of database entry."""
    ...
    def build_default_values(cls, **kwargs):
        ...
        if Vision_IMS.helpers.str2bool(cache.get(cache_key, False)):
    ...
    def all_settings(cls, *, exclude_hidden=False, settings_definition: Union[Dict[str, SettingsKeyType], None] = None, **kwargs):
        ...
        settings: Dict[str, BaseVision_IMSSetting] = {}
        ...
        for key, setting in settings.items():
            ...
            if cls.is_protected(key, **filters) and setting.value != "":
                ...
                setting.value = Vision_IMS.helpers.str2bool(setting.value)
    ...
    def get_setting_object(cls, key, **kwargs):
        """Return an Vision_IMSSetting object matching the given key.
        ...
        if not setting:
            ...
            if Vision_IMS.ready.isImportingData() or not Vision_IMS.ready.canAppAccessDatabase(allow_test=True, allow_shell=True):
                create = False
    ...
    def get_setting(cls, key, backup_value=None, **kwargs):
        ...
        if setting:
            ...
            if setting.is_bool():
                value = Vision_IMS.helpers.str2bool(value)
    ...
    def run_validator(self, validator):
        """Run a validator against the 'value' field for this Vision_IMSSetting object."""
        ...
        if validator is bool:
            ...
            if Vision_IMS.helpers.is_bool(value):
    ...
    def as_bool(self):
        ...
        """
        return Vision_IMS.helpers.str2bool(self.value)
...
def update_exchange_rates(setting):
    ...
    if not Vision_IMS.ready.canAppAccessDatabase():
        return

    Vision_IMS.tasks.update_exchange_rates(force=True)
...
class Vision_IMSSetting(BaseVision_IMSSetting):
    """An Vision_IMSSetting object is a key:value pair used for storing single values (e.g. one-off settings values).
    ...
    class Meta:
        """Meta options for Vision_IMSSetting."""

        verbose_name = "Vision_IMS Setting"
        verbose_name_plural = "Vision_IMS Settings"
    ...
    def save(self, *args, **kwargs):
        ...
        if self.requires_restart() and not Vision_IMS.ready.isImportingData():
            Vision_IMSSetting.set_setting('SERVER_RESTART_REQUIRED', True, None)
    ...
    SETTINGS = {
        ...
        'CURRENCY_UPDATE_PLUGIN': {
            ...
            'default': 'Vision_IMScurrencyexchange'
        },

        'Vision_IMS_DOWNLOAD_FROM_URL': {
        ...
        'Vision_IMS_DOWNLOAD_IMAGE_MAX_SIZE': {
        ...
        'Vision_IMS_DOWNLOAD_FROM_URL_USER_AGENT': {
        ...
        'Vision_IMS_UPDATE_CHECK_INTERVAL': {
        ...
        'Vision_IMS_DELETE_NOTIFICATIONS_DAYS': {
        ...
        'PART_NAME_FORMAT': {
            ...
            'validator': Vision_IMS.validators.validate_part_name_format
        },
        ...
        'PLUGIN_ON_STARTUP': {
            ...
            'default': str(os.getenv('Vision_IMS_DOCKER', False)).lower() in ['1', 'true'],
            'validator': bool,
    ...
    def requires_restart(self):
        ...
        options = Vision_IMSSetting.SETTINGS.get(self.key, None)
...
class Vision_IMSUserSetting(BaseVision_IMSSetting):
    """An Vision_IMSSetting object with a usercontext."""

    class Meta:
        """Meta options for Vision_IMSUserSetting."""

        verbose_name = "Vision_IMS User Setting"
        verbose_name_plural = "Vision_IMS User Settings"
        constraints = [
...
class PriceBreak(MetaMixin):
    ...
    quantity = Vision_IMS.fields.RoundingDecimalField(
        max_digits=15,
        decimal_places=5,
        default=1,
    ...
    price = Vision_IMS.fields.Vision_IMSModelMoneyField(
        max_digits=19,
        decimal_places=6,
        null=True,
...
def get_price(instance, quantity, moq=True, multiples=True, currency=None, break_name: str = 'price_breaks'):
    ...
    if pb_found:
        ...
        return Vision_IMS.helpers.normalize(cost + instance.base_cost)
    else:
        return None
...
class NewsFeedEntry(models.Model):
    """A NewsFeedEntry represents an entry on the RSS/Atom feed that is generated for Vision_IMS news.
...
class CustomUnit(models.Model):
    ...
    def clean(self):
        ...
        from Vision_IMS.conversion import get_unit_registry
...
def after_custom_unit_updated(sender, instance, **kwargs):
    ...
    from Vision_IMS.conversion import reload_unit_registry
