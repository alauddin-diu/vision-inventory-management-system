...
from Vision_IMS.api import BulkDeleteMixin, MetadataView
from Vision_IMS.config import CONFIG_LOOKUPS
from Vision_IMS.filters import ORDER_FILTER, SEARCH_ORDER_FILTER
from Vision_IMS.helpers import inheritors
from Vision_IMS.mixins import (ListAPI, ListCreateAPI, RetrieveAPI,
                              RetrieveUpdateAPI, RetrieveUpdateDestroyAPI)
from Vision_IMS.permissions import IsStaffOrReadOnly, IsSuperuser
...
class CurrencyExchangeView(APIView):
    ...
    def get(self, request, format=None):
        ...
        response = {
            'base_currency': common.models.Vision_IMSSetting.get_setting('Vision_IMS_DEFAULT_CURRENCY', 'USD'),
            'exchange_rates': {},
            'updated': updated,
        }
...
class GlobalSettingsList(SettingsList):
    ...
    queryset = common.models.Vision_IMSSetting.objects.exclude(key__startswith="_")
    serializer_class = common.serializers.GlobalSettingsSerializer

    def list(self, request, *args, **kwargs):
        """Ensure all global settings are created"""

        common.models.Vision_IMSSetting.build_default_values()
...
class GlobalSettingsDetail(RetrieveUpdateAPI):
    ...
    def get_object(self):
        ...
        if key.startswith('_') or key not in common.models.Vision_IMSSetting.SETTINGS.keys():
            raise NotFound()

        return common.models.Vision_IMSSetting.get_setting_object(
            key,
...
class UserSettingsList(SettingsList):
    ...
    queryset = common.models.Vision_IMSUserSetting.objects.all()
    serializer_class = common.serializers.UserSettingsSerializer

    def list(self, request, *args, **kwargs):
        """Ensure all user settings are created"""

        common.models.Vision_IMSUserSetting.build_default_values(user=request.user)
        return super().list(request, *args, **kwargs)
    ...
    def filter_queryset(self, queryset):
        ...
        try:
            ...
            return common.models.Vision_IMSUserSetting.objects.none()
...
class UserSettingsDetail(RetrieveUpdateAPI):
    ...
    def get_object(self):
        ...
        if key.startswith('_') or key not in common.models.Vision_IMSUserSetting.SETTINGS.keys():
            raise NotFound()

        return common.models.Vision_IMSUserSetting.get_setting_object(
            key,
            user=self.request.user,
            cache=False, create=True
        )
