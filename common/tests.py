...
from Vision_IMS.unit_test import (Vision_IMSAPITestCase, Vision_IMSTestCase,
                                 PluginMixin)
from plugin import registry
from plugin.models import NotificationUserSetting

from .api import WebhookView
from .models import (ColorTheme, CustomUnit, Vision_IMSSetting,
                     Vision_IMSUserSetting, NotesImage, NotificationEntry,
...
class SettingsTest(Vision_IMSTestCase):
    ...
    def test_settings_objects(self):
        ...
        settings = Vision_IMSSetting.objects.all()

        self.assertTrue(settings.count() >= 2)

        instance_name = Vision_IMSSetting.objects.get(pk=1)
        self.assertEqual(instance_name.key, 'Vision_IMS_INSTANCE')
        self.assertEqual(instance_name.value, 'My very first Vision_IMS Instance')

        # Check object lookup (case insensitive)
        self.assertEqual(Vision_IMSSetting.get_setting_object('Vision_IMS_inSTanCE').pk, 1)

    def test_settings_functions(self):
        """Test settings functions and properties."""
        # define settings to check
        instance_ref = 'Vision_IMS_INSTANCE'
        instance_obj = Vision_IMSSetting.get_setting_object(instance_ref, cache=False)

        stale_ref = 'STOCK_STALE_DAYS'
        stale_days = Vision_IMSSetting.get_setting_object(stale_ref, cache=False)

        report_size_obj = Vision_IMSSetting.get_setting_object('REPORT_DEFAULT_PAGE_SIZE')
        report_test_obj = Vision_IMSSetting.get_setting_object('REPORT_ENABLE_TEST_REPORT')
        ...
        # check as_choice
        self.assertEqual(instance_obj.as_choice(), 'My very first Vision_IMS Instance')
        ...
        self.assertEqual(instance_obj.as_int(), 'Vision_IMS')  # not an int -> return default
    ...
    def test_allValues(self):
        ...
        # check a few keys
        result = Vision_IMSSetting.allValues()
        self.assertIn('Vision_IMS_INSTANCE', result)
        ...
        result = Vision_IMSSetting.allValues(exclude_hidden=True)
        self.assertNotIn('SERVER_RESTART_REQUIRED', result)

    def test_all_settings(self):
        """Make sure that the all_settings function returns correctly"""
        result = Vision_IMSSetting.all_settings()
        self.assertIn("Vision_IMS_INSTANCE", result)
        self.assertIsInstance(result['Vision_IMS_INSTANCE'], Vision_IMSSetting)

    @mock.patch("common.models.Vision_IMSSetting.get_setting_definition")
    def test_check_all_settings(self, get_setting_definition):
        ...
        self.assertEqual(Vision_IMSSetting.check_all_settings(settings_definition=settings_definition), (False, ["AB", "CD"]))
        Vision_IMSSetting.set_setting('AB', "hello", self.user)
        Vision_IMSSetting.set_setting('CD', "world", self.user)
        self.assertEqual(Vision_IMSSetting.check_all_settings(), (True, []))
    ...
    def test_settings_validator(self, get_setting_definition):
        ...
        with self.assertRaises(ValidationError):
            Vision_IMSSetting.set_setting("AB", "world", self.user)
    ...
    def test_setting_data(self):
        ...
        """
        for key, setting in Vision_IMSSetting.SETTINGS.items():
        ...
        for key, setting in Vision_IMSUserSetting.SETTINGS.items():
            try:
                self.run_settings_check(key, setting)
            except Exception as exc:  # pragma: no cover
    ...
    def test_defaults(self):
        ...
        for key in Vision_IMSSetting.SETTINGS.keys():

            value = Vision_IMSSetting.get_setting_default(key)

            Vision_IMSSetting.set_setting(key, value, self.user)

            self.assertEqual(value, Vision_IMSSetting.get_setting(key))

            # Any fields marked as 'boolean' must have a default value specified
            setting = Vision_IMSSetting.get_setting_object(key)
    ...
    def test_global_setting_caching(self):
        ...
        cache_key = Vision_IMSSetting.create_cache_key(key)
        self.assertEqual(cache_key, 'Vision_IMSSetting:PART_NAME_FORMAT')
        ...
        val = Vision_IMSSetting.get_setting(key)
        ...
        for val in ['A', '{{ part.IPN }}', 'C']:
            ...
            Vision_IMSSetting.set_setting(key, val, None)
            self.assertEqual(cache.get(cache_key).value, val)
            self.assertEqual(Vision_IMSSetting.get_setting(key), val)
    ...
    def test_user_setting_caching(self):
        ...
        for user in get_user_model().objects.all():
            setting = Vision_IMSUserSetting.get_setting_object(key, user=user)
            cache_key = setting.cache_key
            self.assertEqual(cache_key, f"Vision_IMSUserSetting:SEARCH_PREVIEW_RESULTS_user:{user.username}")
            Vision_IMSUserSetting.set_setting(key, user.pk, None, user=user)
            self.assertIsNotNone(cache.get(cache_key))

        # Iterate through a second time, ensure the values have been cached correctly
        for user in get_user_model().objects.all():
            value = Vision_IMSUserSetting.get_setting(key, user=user)
            self.assertEqual(value, user.pk)


class GlobalSettingsApiTest(Vision_IMSAPITestCase):
    ...
    def test_global_settings_api_list(self):
        ...
        for key in Vision_IMSSetting.SETTINGS:
            Vision_IMSSetting.get_setting_object(key, cache=False)

        response = self.get(url, expected_code=200)

        n_public_settings = len([k for k in Vision_IMSSetting.SETTINGS.keys() if not k.startswith('_')])
    ...
    def test_company_name(self):
        ...
        setting = Vision_IMSSetting.get_setting_object('Vision_IMS_COMPANY_NAME')
    ...
    def test_api_detail(self):
        ...
        setting = Vision_IMSSetting.objects.get(key=key)
...
class UserSettingsApiTest(Vision_IMSAPITestCase):
    ...
    def test_user_setting_init(self):
        ...
        self.assertFalse(Vision_IMSUserSetting.objects.filter(key=key).exists())
        ...
        setting = Vision_IMSUserSetting.objects.get(key=key, user=self.user)
    ...
    def test_user_setting_boolean(self):
        ...
        setting = Vision_IMSUserSetting.get_setting_object(
...
class NotificationUserSettingsApiTest(Vision_IMSAPITestCase):
...
class PluginSettingsApiTest(PluginMixin, Vision_IMSAPITestCase):
...
class NotificationTest(Vision_IMSAPITestCase):
    ...
    def test_api_list(self):
        ...
        # Ref: https://github.com/Vision_IMS/Vision_IMS/pull/3154
...
class CommonTest(Vision_IMSAPITestCase):
    ...
    def test_restart_flag(self):
        ...
        # set flag true
        common.models.Vision_IMSSetting.set_setting('SERVER_RESTART_REQUIRED', True, None)

        # reload the app
        registry.reload_plugins()

        # now it should be false again
        self.assertFalse(common.models.Vision_IMSSetting.get_setting('SERVER_RESTART_REQUIRED'))
    ...
    def test_config_api(self):
        ...
        data = [
            ...
            self.get(reverse('api-config-detail', kwargs={'key': 'Vision_IMS_DEBUG'}), expected_code=200).data,  # detail endpoint
        ]

        for item in data:
            self.assertEqual(item['key'], 'Vision_IMS_DEBUG')
            self.assertEqual(item['env_var'], 'Vision_IMS_DEBUG')
...
class CurrencyAPITests(Vision_IMSAPITestCase):
...
class NotesImageTest(Vision_IMSAPITestCase):
...
class ProjectCodesTest(Vision_IMSAPITestCase):
...
class CustomUnitAPITest(Vision_IMSAPITestCase):
