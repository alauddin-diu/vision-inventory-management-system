...
from plugin.builtin.integration.core_notifications import \
    Vision_IMSCoreNotificationsPlugin
...
class CoreNotificationTestTests(BaseNotificationIntegrationTest):
    ...
    def test_email(self):
        ...
        plugin = registry.plugins.get('Vision_IMScorenotificationsplugin')
        ...
        NotificationUserSetting.set_setting(
            ...
            user=self.user,
            method=Vision_IMSCoreNotificationsPlugin.EmailNotification.METHOD_NAME
        )

        # run through
        self._notification_run(Vision_IMSCoreNotificationsPlugin.EmailNotification)
