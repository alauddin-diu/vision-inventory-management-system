...
The main code for plugin special sauce is in the plugin registry in `Vision_IMS/plugin/registry.py`.
"""
...
class PluginAppConfig(AppConfig):
    ...
    def ready(self):
        ...
        if not canAppAccessDatabase(allow_test=True, allow_plugins=True):
            ...
            logger.info('Loading Vision_IMS plugins')

            if not registry.is_loading:
                # this is the first startup
                try:
                    from common.models import Vision_IMSSetting
                    if Vision_IMSSetting.get_setting('PLUGIN_ON_STARTUP', create=False, cache=False):
