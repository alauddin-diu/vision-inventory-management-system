"""Base Class for Vision_IMS plugins."""
...
class VersionMixin:
    ...
    def check_version(self, latest=None) -> bool:
        """Check if plugin functions for the current Vision_IMS version."""
        from Vision_IMS import version

        latest = latest if latest else version.Vision_IMSVersionTuple()
        min_v = version.Vision_IMSVersionTuple(self.MIN_VERSION)
        max_v = version.Vision_IMSVersionTuple(self.MAX_VERSION)

        return bool(min_v <= latest <= max_v)


class Vision_IMSPlugin(VersionMixin, MixinBase, MetaBase):
    """The Vision_IMSPlugin class is used to integrate with 3rd party software.

    DO NOT USE THIS DIRECTLY, USE plugin.Vision_IMSPlugin
    """
