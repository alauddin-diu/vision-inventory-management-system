...
class Vision_IMSPluginTests(TestCase):
    """Tests for Vision_IMSPlugin."""
    ...
    def setUpTestData(cls):
        ...
        cls.plugin = Vision_IMSPlugin()

        class NamedPlugin(Vision_IMSPlugin):
            """a named plugin."""
            NAME = 'abc123'

        cls.named_plugin = NamedPlugin()

        class SimpleVision_IMSPlugin(Vision_IMSPlugin):
            NAME = 'SimplePlugin'

        cls.plugin_simple = SimpleVision_IMSPlugin()

        class OldVision_IMSPlugin(Vision_IMSPlugin):
            PLUGIN_SLUG = 'old'

        cls.plugin_old = OldVision_IMSPlugin()

        class NameVision_IMSPlugin(Vision_IMSPlugin):
            NAME = 'Aplugin'
            SLUG = 'a'
            TITLE = 'a title'
        ...
        cls.plugin_name = NameVision_IMSPlugin()
        cls.plugin_sample = SampleIntegrationPlugin()

        class VersionVision_IMSPlugin(Vision_IMSPlugin):
            NAME = 'Version'
            SLUG = 'testversion'
        ...
        cls.plugin_version = VersionVision_IMSPlugin()
...
class RegistryTests(TestCase):
    ...
    def run_package_test(self, directory):
        ...
        envs = {'Vision_IMS_PLUGIN_TEST_DIR': directory}
        with mock.patch.dict(os.environ, envs):
            # Reload to rediscover plugins
            registry.reload_plugins(full_reload=True, collect=True)

            # Depends on the meta set in Vision_IMS/plugin/mock/simple:SimplePlugin
    ...
    def test_custom_loading(self):
        ...
        # Patch env
        envs = {'Vision_IMS_PLUGIN_TEST_DIR': 'plugin_test_dir'}
        with mock.patch.dict(os.environ, envs):
    ...
    def test_package_loading(self):
        ...
        subprocess.check_output('pip install Vision_IMS-zapier'.split())
        ...
        self.assertEqual(plg.name, 'Vision_IMS_zapier')
    ...
    def test_broken_samples(self):
        ...
        with mock.patch.dict(os.environ, {'Vision_IMS_PLUGIN_TEST_DIR': brokenDir}):
            # Reload to rediscover plugins
            registry.reload_plugins(full_reload=True, collect=True)
