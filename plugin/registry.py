...
from Vision_IMS.config import get_plugin_dir
from Vision_IMS.ready import canAppAccessDatabase
...
class PluginsRegistry:
    ...
    def __init__(self) -> None:
        ...
        """
        # plugin registry
        self.plugins: Dict[str, Vision_IMSPlugin] = {}           # List of active instances
        self.plugins_inactive: Dict[str, Vision_IMSPlugin] = {}  # List of inactive instances
        self.plugins_full: Dict[str, Vision_IMSPlugin] = {}      # List of all plugin instances

        # Keep an internal hash of the plugin registry state
        self.registry_hash = None

        self.plugin_modules: List[Vision_IMSPlugin] = []         # Holds all discovered plugins
    ...
    def plugin_dirs(self):
        ...
        if settings.PLUGINS_ENABLED:
            ...
            if settings.TESTING:
                custom_dirs = os.getenv('Vision_IMS_PLUGIN_TEST_DIR', None)
            ...
            if custom_dirs is not None:
                ...
                for pd_text in custom_dirs.split(','):
                    ...
                    if not init_filename.exists():
                        try:
                            init_filename.write_text("# Vision_IMS plugin directory\n")
                        except Exception:  # pragma: no cover
    ...
    def collect_plugins(self):
        ...
        for plugin in self.plugin_dirs():
            ...
            modules = get_plugins(raw_module, Vision_IMSPlugin, path=parent_path)
        ...
        for plg in self.plugin_modules:
            ...
            if settings.PLUGIN_TESTING or builtin or (plg_db and plg_db.active):
                ...
                if (plg_i.MIN_VERSION or plg_i.MAX_VERSION) and not plg_i.check_version():
                    ...
                    p = plg_name
                    v = version.Vision_IMSVersion()
                    _msg = _(f"Plugin '{p}' is not compatible with the current Vision_IMS version {v}")
    ...
    def _clean_registry(self):
        """Remove all plugins from registry."""
        self.plugins: Dict[str, Vision_IMSPlugin] = {}
        self.plugins_inactive: Dict[str, Vision_IMSPlugin] = {}
        self.plugins_full: Dict[str, Vision_IMSPlugin] = {}
    ...
    def _update_urls(self):
        ...
        from Vision_IMS.urls import urlpatterns
        ...
        for index, url in enumerate(urlpatterns):
            ...
            if app_name == 'admin':
                urlpatterns[index] = re_path(r'^admin/', admin.site.urls, name='Vision_IMS-admin')
            if app_name == 'plugin':
    ...
    def update_plugin_hash(self):
        ...
        try:
            old_hash = Vision_IMSSetting.get_setting("_PLUGIN_REGISTRY_HASH", "", create=False, cache=False)
        except Exception:
            old_hash = ""
        ...
        if old_hash != self.registry_hash:
            try:
                ...
                Vision_IMSSetting.set_setting("_PLUGIN_REGISTRY_HASH", self.registry_hash, change_user=None)
    ...
    def calculate_plugin_hash(self):
        ...
        for k in keys:
            try:
                data.update(str(Vision_IMSSetting.get_setting(k, False, cache=False, create=False)).encode())
            except Exception:
                pass
    ...
    def check_reload(self):
        ...
        try:
            reg_hash = Vision_IMSSetting.get_setting("_PLUGIN_REGISTRY_HASH", "", create=False, cache=False)
        except Exception as exc:
