...
class SettingsMixinTest(BaseMixinDefinition, Vision_IMSTestCase):
    ...
    def setUp(self):
        """Setup for all tests."""
        class SettingsCls(SettingsMixin, Vision_IMSPlugin):
            SETTINGS = self.TEST_SETTINGS
        self.mixin = SettingsCls()

        class NoSettingsCls(SettingsMixin, Vision_IMSPlugin):
            pass
...
class UrlsMixinTest(BaseMixinDefinition, TestCase):
    ...
    def setUp(self):
        """Setup for all tests."""
        class UrlsCls(UrlsMixin, Vision_IMSPlugin):
            def test():
                return 'ccc'
            URLS = [re_path('testpath', test, name='test'), ]
        self.mixin = UrlsCls()

        class NoUrlsCls(UrlsMixin, Vision_IMSPlugin):
            pass
...
class AppMixinTest(BaseMixinDefinition, TestCase):
    ...
    def setUp(self):
        """Setup for all tests."""
        class TestCls(AppMixin, Vision_IMSPlugin):
            pass
        self.mixin = TestCls()
...
class NavigationMixinTest(BaseMixinDefinition, TestCase):
    ...
    def setUp(self):
        """Setup for all tests."""
        class NavigationCls(NavigationMixin, Vision_IMSPlugin):
            NAVIGATION = [
        ...
        class NothingNavigationCls(NavigationMixin, Vision_IMSPlugin):
            pass
...
class APICallMixinTest(BaseMixinDefinition, TestCase):
    ...
    def setUp(self):
        ...
        class MixinCls(APICallMixin, SettingsMixin, Vision_IMSPlugin):
            ...
            def get_external_url(self, simple: bool = True):
                ...
                return self.api_call('orgs/Vision_IMS', simple_response=simple)
        ...
        class WrongCLS(APICallMixin, Vision_IMSPlugin):
            pass

        self.mixin_wrong = WrongCLS()

        class WrongCLS2(APICallMixin, Vision_IMSPlugin):
    ...
    def test_api_call(self):
        ...
        result = self.mixin.api_call('/orgs/Vision_IMS', simple_response=False)
        self.assertTrue(result)
        self.assertEqual(result.reason, 'OK')

        # api_call with filter
        result = self.mixin.api_call('repos/Vision_IMS/Vision_IMS/stargazers', url_args={'page': '2'})
        self.assertTrue(result)
...
class PanelMixinTests(Vision_IMSTestCase):
    ...
    def test_mixin(self):
        ...
        with self.assertRaises(MixinNotImplementedError):
            class Wrong(PanelMixin, Vision_IMSPlugin):
                pass
