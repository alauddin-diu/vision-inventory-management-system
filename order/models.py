...
from Vision_IMS.exceptions import log_error
from Vision_IMS.fields import (Vision_IMSModelMoneyField, Vision_IMSURLField,
                              RoundingDecimalField)
from Vision_IMS.helpers import decimal2string
from Vision_IMS.helpers_model import getSetting, notify_responsible
from Vision_IMS.models import (Vision_IMSAttachment, Vision_IMSBarcodeMixin,
                              Vision_IMSNotesMixin, MetadataMixin,
                              ReferenceIndexingMixin)
from Vision_IMS.status_codes import (PurchaseOrderStatus,
...
class TotalPriceMixin(models.Model):
    ...
    total_price = Vision_IMSModelMoneyField(
    ...
    order_currency = models.CharField(
        ...
        validators=[Vision_IMS.validators.validate_currency_code]
    )
...
class Order(Vision_IMSBarcodeMixin, Vision_IMSNotesMixin, MetadataMixin, ReferenceIndexingMixin):
    ...
    link = Vision_IMSURLField(blank=True, verbose_name=_('Link'), help_text=_('Link to external page'))
...
class PurchaseOrder(TotalPriceMixin, Order):
    ...
    def place_order(self):
        ...
        if self.status == PurchaseOrderStatus.PENDING:
            ...
            notify_responsible(
                ...
                content=Vision_IMSNotificationBodies.NewOrder
            )
    ...
    def receive_line_item(self, line, location, quantity, user, status=StockStatus.OK.value, **kwargs):
        ...
        try:
            ...
            quantity = Vision_IMS.helpers.clean_decimal(quantity)
        except TypeError:
            raise ValidationError({
        ...
        notify_responsible(
            ...
            PurchaseOrder,
            exclude=user,
            content=Vision_IMSNotificationBodies.ItemsReceived,
        )
...
def after_save_sales_order(sender, instance: SalesOrder, created: bool, **kwargs):
    ...
    """
    if not Vision_IMS.ready.canAppAccessDatabase(allow_test=True) or Vision_IMS.ready.isImportingData():
        return
...
class PurchaseOrderAttachment(Vision_IMSAttachment):
...
class SalesOrderAttachment(Vision_IMSAttachment):
...
class OrderExtraLine(OrderLineItem):
    ...
    price = Vision_IMSModelMoneyField(
        max_digits=19,
        decimal_places=6,
        null=True, blank=True,
        allow_negative=True,
...
class SalesOrderLineItem(OrderLineItem):
    ...
    sale_price = Vision_IMSModelMoneyField(
        max_digits=19,
        decimal_places=6,
        null=True, blank=True,
...
class SalesOrderShipment(Vision_IMSNotesMixin, MetadataMixin, models.Model):
...
class ReturnOrder(TotalPriceMixin, Order):
    ...
    def receive_line_item(self, line, location, user, note=''):
        ...
        notify_responsible(
            ...
            ReturnOrder,
            exclude=user,
            content=Vision_IMSNotificationBodies.ReturnOrderItemsReceived,
        )
...
class ReturnOrderAttachment(Vision_IMSAttachment):
