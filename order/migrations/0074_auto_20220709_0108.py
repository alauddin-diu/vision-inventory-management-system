...
def update_salesorder_reference(apps, schema_editor):
    ...
    try:
        prefix = Vision_IMSSetting.objects.get(key='SALESORDER_REFERENCE_PREFIX').value
    except Exception:
        prefix = 'SO-'
    ...
    try:
        setting = Vision_IMSSetting.objects.get(key='SALESORDER_REFERENCE_PATTERN')
...
def update_purchaseorder_reference(apps, schema_editor):
    ...
    try:
        prefix = Vision_IMSSetting.objects.get(key='PURCHASEORDER_REFERENCE_PREFIX').value
    except Exception:
        prefix = 'PO-'
    ...
    # Create or update the BuildOrder.reference pattern
    try:
        setting = Vision_IMSSetting.objects.get(key='PURCHASEORDER_REFERENCE_PATTERN')
        setting.value = pattern
        setting.save()
    except Vision_IMSSetting.DoesNotExist:
        setting = Vision_IMSSetting.objects.create(
