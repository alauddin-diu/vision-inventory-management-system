...
from Vision_IMS.helpers import (extract_serial_numbers, hash_barcode, normalize,
                               str2bool)
from Vision_IMS.serializers import (Vision_IMSAttachmentSerializer,
                                   Vision_IMSCurrencySerializer,
                                   Vision_IMSDecimalField,
                                   Vision_IMSModelSerializer,
                                   Vision_IMSMoneySerializer)
from Vision_IMS.status_codes import (PurchaseOrderStatusGroups,
...
class TotalPriceMixin(serializers.Serializer):
    ...
    total_price = Vision_IMSMoneySerializer(
        allow_null=True,
        read_only=True,
    )

    order_currency = Vision_IMSCurrencySerializer(
        allow_blank=True,
        allow_null=True,
        required=False,
...
class AbstractExtraLineSerializer(serializers.Serializer):
    ...
    price = Vision_IMSMoneySerializer(
        allow_null=True
    )

    price_currency = Vision_IMSCurrencySerializer()
...
class PurchaseOrderSerializer(TotalPriceMixin, AbstractOrderSerializer, Vision_IMSModelSerializer):
...
class PurchaseOrderLineItemSerializer(Vision_IMSModelSerializer):
    ...
    purchase_price = Vision_IMSMoneySerializer(allow_null=True)

    destination_detail = stock.serializers.LocationBriefSerializer(source='get_destination', read_only=True)

    purchase_price_currency = Vision_IMSCurrencySerializer(help_text=_('Purchase price currency'))
...
class PurchaseOrderExtraLineSerializer(AbstractExtraLineSerializer, Vision_IMSModelSerializer):
...
class PurchaseOrderAttachmentSerializer(Vision_IMSAttachmentSerializer):
    ...
    class Meta:
        ...
        fields = Vision_IMSAttachmentSerializer.attachment_fields([
            'order',
        ])


class SalesOrderSerializer(TotalPriceMixin, AbstractOrderSerializer, Vision_IMSModelSerializer):
...
class SalesOrderAllocationSerializer(Vision_IMSModelSerializer):
...
class SalesOrderLineItemSerializer(Vision_IMSModelSerializer):
    ...
    quantity = Vision_IMSDecimalField()

    allocated = serializers.FloatField(source='allocated_quantity', read_only=True)

    shipped = Vision_IMSDecimalField(read_only=True)

    sale_price = Vision_IMSMoneySerializer(allow_null=True)

    sale_price_currency = Vision_IMSCurrencySerializer(help_text=_('Sale price currency'))


class SalesOrderShipmentSerializer(Vision_IMSModelSerializer):
...
class SalesOrderExtraLineSerializer(AbstractExtraLineSerializer, Vision_IMSModelSerializer):
...
class SalesOrderAttachmentSerializer(Vision_IMSAttachmentSerializer):
...
class ReturnOrderSerializer(AbstractOrderSerializer, TotalPriceMixin, Vision_IMSModelSerializer):
...
class ReturnOrderLineItemSerializer(Vision_IMSModelSerializer):
    ...
    price = Vision_IMSMoneySerializer(allow_null=True)
    price_currency = Vision_IMSCurrencySerializer(help_text=_('Line price currency'))


class ReturnOrderExtraLineSerializer(AbstractExtraLineSerializer, Vision_IMSModelSerializer):
...
class ReturnOrderAttachmentSerializer(Vision_IMSAttachmentSerializer):
