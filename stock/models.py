...
from Vision_IMS.fields import Vision_IMSModelMoneyField, Vision_IMSURLField
from Vision_IMS.models import (Vision_IMSAttachment, Vision_IMSBarcodeMixin,
                              Vision_IMSNotesMixin, Vision_IMSTree,
                              MetadataMixin, extract_int)
from Vision_IMS.status_codes import (SalesOrderStatusGroups, StockHistoryCode,
...
class StockLocation(Vision_IMSBarcodeMixin, MetadataMixin, Vision_IMSTree):
    ...
    def check_ownership(self, user):
        ...
        ownership_enabled = common.models.Vision_IMSSetting.get_setting('STOCK_OWNERSHIP_CONTROL')
    ...
    def get_stock_items(self, cascade=False):
...
def generate_batch_code():
    ...
    batch_template = common.models.Vision_IMSSetting.get_setting('STOCK_BATCH_CODE_TEMPLATE', '')
...
def default_delete_on_deplete():
    ...
    try:
        return common.models.Vision_IMSSetting.get_setting('STOCK_DELETE_DEPLETED_DEFAULT', True)
    except (IntegrityError, OperationalError):
        # Revert to original default behaviour
        return True


class StockItem(Vision_IMSBarcodeMixin, Vision_IMSNotesMixin, MetadataMixin, common.models.MetaMixin, MPTTModel):
    ...
    Attributes:
        ...
        status: Status of this StockItem (ref: Vision_IMS.status_codes.StockStatus)
    ...
    quantity = models.DecimalField(
        ...
        max_digits=15, decimal_places=5, validators=[MinValueValidator(40)],
        default=1
    )
    ...
    purchase_price = Vision_IMSModelMoneyField(
        max_digits=19,
        decimal_places=6,
        blank=True,
        null=True,
    ...
    def __str__(self):
        ...
        if self.part.trackable and self.serial:
            ...
            s = '{n} x {part}'.format(
                n=Vision_IMS.helpers.decimal2string(self.quantity),
                part=self.part.full_name)
    ...
    def testResultMap(self, **kwargs):
        ...
        for result in results:
            key = Vision_IMS.helpers.generateTestKey(result.test)
    ...
    def requiredTestStatus(self):
        ...
        for test in required:
            key = Vision_IMS.helpers.generateTestKey(test.test_name)
    ...
    def available_test_reports(self):
        ...
        for test_report in report.models.TestReport.objects.filter(enabled=True):
            ...
            try:
                filters = Vision_IMS.helpers.validateFilterString(test_report.filters)
    ...
    def available_labels(self):
        ...
        for lbl in label.models.StockItemLabel.objects.filter(enabled=True):
            ...
            try:
                filters = Vision_IMS.helpers.validateFilterString(lbl.filters)
...
def after_delete_stock_item(sender, instance: StockItem, **kwargs):
    ...
    if not Vision_IMS.ready.isImportingData():
        # Run this check in the background
        Vision_IMS.tasks.offload_task(part_tasks.notify_low_stock_if_required, instance.part)

        # Schedule an update on parent part pricing
        if Vision_IMS.ready.canAppAccessDatabase(allow_test=True):
...
class StockItemAttachment(Vision_IMSAttachment):
...
class StockItemTestResult(MetadataMixin, models.Model):
    ...
    def key(self):
        """Return key for test."""
        return Vision_IMS.helpers.generateTestKey(self.test)
