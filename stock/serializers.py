...
import Vision_IMS.status_codes
import part.models as part_models
import stock.filters
from company.serializers import SupplierPartSerializer
from Vision_IMS.models import extract_int
from Vision_IMS.serializers import (Vision_IMSCurrencySerializer,
...
class LocationBriefSerializer(Vision_IMS.serializers.Vision_IMSModelSerializer):
...
class StockItemTestResultSerializer(Vision_IMS.serializers.Vision_IMSModelSerializer):
    ...
    user_detail = Vision_IMS.serializers.UserSerializer(source='user', read_only=True)

    key = serializers.CharField(read_only=True)

    attachment = Vision_IMS.serializers.Vision_IMSAttachmentSerializerField(required=False)


class StockItemSerializerBrief(Vision_IMS.serializers.Vision_IMSModelSerializer):
...
class StockItemSerializer(Vision_IMS.serializers.Vision_IMSTagModelSerializer):
    ...
    def annotate_queryset(queryset):
        ...
        stale_days = common.models.Vision_IMSSetting.get_setting('STOCK_STALE_DAYS')
    ...
    purchase_price = Vision_IMS.serializers.Vision_IMSMoneySerializer(
        label=_('Purchase Price'),
        allow_null=True,
        help_text=_('Purchase price of this stock item, per unit or pack'),
    )

    purchase_price_currency = Vision_IMSCurrencySerializer(help_text=_('Purchase currency of this stock item'))
...
class SerializeStockItemSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        try:
            serials = Vision_IMS.helpers.extract_serial_numbers(
                serial_numbers,
                quantity,
...
class StockChangeStatusSerializer(serializers.Serializer):
    ...
    status = serializers.ChoiceField(
        choices=Vision_IMS.status_codes.StockStatus.items(),
        default=Vision_IMS.status_codes.StockStatus.OK.value,
        label=_('Status'),
    )
    ...
    def save(self):
        ...
        for item in items:
            ...
            transaction_notes.append(
                StockItemTracking(
                    item=item,
                    tracking_type=Vision_IMS.status_codes.StockHistoryCode.EDITED.value,
                    date=now,
                    deltas=deltas,
                    user=user,
                    notes=note,
...
class StockLocationTypeSerializer(Vision_IMS.serializers.Vision_IMSModelSerializer):
...
class LocationTreeSerializer(Vision_IMS.serializers.Vision_IMSModelSerializer):
...
class LocationSerializer(Vision_IMS.serializers.Vision_IMSTagModelSerializer):
...
class StockItemAttachmentSerializer(Vision_IMS.serializers.Vision_IMSAttachmentSerializer):
...
class StockTrackingSerializer(Vision_IMS.serializers.Vision_IMSModelSerializer):
    ...
    user_detail = Vision_IMS.serializers.UserSerializer(source='user', many=False, read_only=True)
