...
from Vision_IMS.fields import Vision_IMSURLField, RoundingDecimalField
...
class Company(Vision_IMSNotesMixin, MetadataMixin, models.Model):
    ...
    link = Vision_IMSURLField(blank=True, verbose_name=_('Link'), help_text=_('Link to external company information'))
    ...
    def get_image_url(self):
        ...
        if self.image:
            return Vision_IMS.helpers.getMediaUrl(self.image.url)
        else:
            return Vision_IMS.helpers.getBlankImage()

    def get_thumbnail_url(self):
        """Return the URL for the thumbnail image for this Company."""
        if self.image:
            return Vision_IMS.helpers.getMediaUrl(self.image.thumbnail.url)
        else:
            return Vision_IMS.helpers.getBlankThumbnail()
...
class CompanyAttachment(Vision_IMSAttachment):
...
class Address(models.Model):
    ...
    link = Vision_IMSURLField(blank=True,
                             verbose_name=_('Link'),
...
class ManufacturerPartAttachment(Vision_IMSAttachment):
...
class SupplierPart(MetadataMixin, Vision_IMSBarcodeMixin, common.models.MetaMixin):
    ...
    def clean(self):
        ...
        if self.pack_quantity and self.part:
            try:
                ...
                native_value = Vision_IMS.conversion.convert_physical_value(
                    self.pack_quantity, self.part.units,
                    strip_units=False
                )

                # If part units are not provided, value must be dimensionless
                if not self.part.units and not Vision_IMS.conversion.is_dimensionless(native_value):
