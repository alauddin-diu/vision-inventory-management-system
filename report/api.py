...
import order.models
import part.models
from Vision_IMS.api import MetadataView
from Vision_IMS.exceptions import log_error
from Vision_IMS.filters import Vision_IMSSearchFilter
from Vision_IMS.mixins import ListAPI, RetrieveAPI, RetrieveUpdateDestroyAPI
    ...
    def filter_queryset(self, queryset):
        ...
        if len(items) > 0:
            ...
            for report in queryset.all():
                ...
                try:
                    filters = Vision_IMS.helpers.validateFilterString(report.filters)
...
class ReportPrintMixin:
    ...
    def print(self, request, items_to_print):
        ...
        debug_mode = common.models.Vision_IMSSetting.get_setting('REPORT_DEBUG_MODE', cache=False)
        ...
        try:
            ...
            if debug_mode:
                ...
                inline = common.models.Vision_IMSUserSetting.get_setting('REPORT_INLINE', user=request.user, cache=False)

                return Vision_IMS.helpers.DownloadFile(
                    pdf,
                    report_name,
...
class StockItemTestReportPrint(StockItemTestReportMixin, ReportPrintMixin, RetrieveAPI):
    ...
    def report_callback(self, item, report, request):
        ...
        if common.models.Vision_IMSSetting.get_setting('REPORT_ATTACH_TEST_REPORT', cache=False):
