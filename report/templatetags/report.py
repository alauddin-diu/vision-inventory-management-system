...
def asset(filename):
    ...
    debug_mode = Vision_IMSSetting.get_setting('REPORT_DEBUG_MODE')
...
def uploaded_image(filename, replace_missing=True, replacement_file='blank_image.png', validate=True):
    ...
    if exists and validate and not Vision_IMS.helpers.TestIfImage(full_path):
...
def logo_image(**kwargs):
    ...
    - Otherwise, return a path to the default Vision_IMS logo
    """

    # If in debug mode, return URL to the image, not a local file
    debug_mode = Vision_IMSSetting.get_setting('REPORT_DEBUG_MODE')

    return Vision_IMS.helpers.getLogoImage(as_file=not debug_mode, **kwargs)


@register.simple_tag()
def internal_link(link, text):
    """Make a <a></a> href which points to an Vision_IMS URL.

    Uses the Vision_IMS.helpers_model.construct_absolute_url function to build the URL.
    """
    text = str(text)

    url = Vision_IMS.helpers_model.construct_absolute_url(link)
