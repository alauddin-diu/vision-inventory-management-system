...
function constructOrderTableButtons(options={}) {
    ...
    if (!display_mode) {
        display_mode = Vision_IMSLoad(key, 'list');
    }
 ...
 * - The table has been loaded using the Vision_IMSTable() function, not bootstrapTable()
...
/*
 * Convert bootstrap-table style parameters to "Vision_IMS" style
*/
function convertQueryParameters(params, filters) {
    ...
    // Add "order" back in (if it was originally specified by Vision_IMS)
    // Annoyingly, "order" shadows some field names in Vision_IMS...
    if ('order' in filters) {
 ...
 */
$.fn.Vision_IMSTable = function(options) {
    ...
    if (!options.disablePagination) {
        ...
        options.pageSize = options.pageSize || Vision_IMSLoad(varName, 25);
    ...
    options.onColumnSwitch = function() {
        ...
        Vision_IMSSave(`table_columns_${tableName}`, text);
    };
    ...
    var visibleColumns = Vision_IMSLoad(`table_columns_${tableName}`, null);
