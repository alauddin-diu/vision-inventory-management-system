...
function createNewModal(options={}) {
    ...
    $('.Vision_IMS-modal').each(function() {
    ...
    var html = `
    <div class='modal fade modal-fixed-footer modal-primary Vision_IMS-modal' role='dialog' id='modal-form-${id}' tabindex='-1'>
    ...
    $(modal_name).on('hidden.bs.modal', function() {
        ...
        $('.Vision_IMS-modal').last().css("z-index", 10000);
    });
...
function reloadFieldOptions(fieldName, options) {
    ...
    Vision_IMSGet(options.url, options.params, {
    ...
    var ajax_data = {
        ...
        error: function(xhr) {
            ...
            if (xhr.status == 0) {
                ...
                showAlertDialog(
                    '{% trans "No Response" %}',
                    '{% trans "No response from the Vision_IMS server" %}',
                );
