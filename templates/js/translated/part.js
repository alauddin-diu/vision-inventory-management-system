...
function duplicatePart(pk, options={}) {
    ...
    Vision_IMSGet(`{% url "api-part-list" %}${pk}/`, {}, {
...
function partStockLabel(part, options={}) {
    ...
    if (options.showDemandInfo) {
        ...
        Vision_IMSGet(`{% url "api-part-list" %}${part.pk}/requirements/`, {}, {
            async: false,
            success: function(response) {
        ...
        template: {
            ...
            onEdit: function(value, name, field, opts) {
                ...
                if (value) {
                    ...
                    Vision_IMSGet(`{% url "api-part-parameter-template-list" %}${value}/`, {}, {
                        async: false,
                        success: function(response) {
...
function loadParametricPartTable(table, options={}) {
    ...
    $(table).Vision_IMSTable({
        ...
        // Ref: https://github.com/Vision_IMS/Vision_IMS/issues/4851
        // filterControl: true,
...
function loadPartTable(table, url, options={}) {
    ...
    var grid_view = options.gridView && Vision_IMSLoad('part-grid-view') == 1;
    ...
    $(table).Vision_IMSTable({
        ...
        showFooter: true,
        onPostBody: function() {
            grid_view = Vision_IMSLoad('part-grid-view') == 1;
            if (grid_view) {
        ...
        buttons: options.gridView ? [
            {
                ...
                event: () => {
                    Vision_IMSSave('part-grid-view', 0);
            ...
            },
            {
                ...
                event: () => {
                    Vision_IMSSave('part-grid-view', 1);
                    $(table).bootstrapTable(
                        'refreshOptions',
                        {
                            showCustomView: true,
...
function loadPartCategoryTable(table, options) {
    ...
    var tree_view = options.allowTreeView && Vision_IMSLoad('category-tree-view') == 1;

    if (tree_view) {
        params.cascade = true;
        params.depth = global_settings.Vision_IMS_TREE_DEPTH;
    }
    ...
    table.Vision_IMSTable({
        ...
        buttons: options.allowTreeView ? [
            {
                ...
                event: () => {
                    Vision_IMSSave('category-tree-view', 0);
            ...
            },
            {
                ...
                event: () => {
                    Vision_IMSSave('category-tree-view', 1);
        ...
        onPostBody: function() {
            ...
            if (options.allowTreeView) {
                ...
                tree_view = Vision_IMSLoad('category-tree-view') == 1;
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row._level >= global_settings.Vision_IMS_TREE_DEPTH && !row.subReceived) {
                        if (row.subRequested) {
...
function loadPartSchedulingChart(canvas_id, part_id) {
    ...
    Vision_IMSGet(`{% url "api-part-list" %}${part_id}/`, {}, {
        async: false,
        success: function(response) {
            part_info = response;
        }
