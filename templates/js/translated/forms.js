...
function constructFormBody(fields, options) {
    ...
    if (options.confirm && global_settings.Vision_IMS_REQUIRE_CONFIRM) {
...
function submitFormData(fields, options) {
    ...
    var upload_func = Vision_IMSPut;

    if (has_files) {
        upload_func = Vision_IMSFormDataUpload;
        data = form_data;
    }
...
function initializeRelatedField(field, fields, options={}) {
    ...
    select.select2({
        ...
        ajax: {
            ...
            processResults: function(response) {
                // Convert the returned Vision_IMS data into select2-friendly format
    ...
    if (field.value) {
        ...
        Vision_IMSGet(url, field.filters || {}, {
            success: function(data) {
        ...
        Vision_IMSGet(field.api_url, filters || {}, {
            success: function(data) {
    ...
    if(field.tree_picker) {
        ...
        button.on("click", () => {
            ...
            Vision_IMSGet(field.tree_picker.url, {}, {
                success: (data) => {
 ...
 * - model: The name of the Vision_IMS model e.g. 'stockitem'
