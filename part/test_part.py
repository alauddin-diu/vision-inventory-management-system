...
from Vision_IMS import version
...
from .models import (Part, PartCategory, PartCategoryStar, PartRelated,
                     ...
                     rename_part_image)
from .templatetags import vision_ims_extras


class TemplateTagTest(Vision_IMSTestCase):
    """Tests for the custom template tag code."""

    def test_define(self):
        """Test the 'define' template tag"""
        self.assertEqual(int(vision_ims_extras.define(3)), 3)

    def test_str2bool(self):
        """Various test for the str2bool template tag"""
        self.assertEqual(int(vision_ims_extras.str2bool('true')), True)
        self.assertEqual(int(vision_ims_extras.str2bool('yes')), True)
        self.assertEqual(int(vision_ims_extras.str2bool('none')), False)
        self.assertEqual(int(vision_ims_extras.str2bool('off')), False)

    def test_add(self):
        """Test that the 'add"""
        self.assertEqual(int(vision_ims_extras.add(3, 5)), 8)

    def test_plugins_enabled(self):
        """Test the plugins_enabled tag"""
        self.assertEqual(vision_ims_extras.plugins_enabled(), True)

    def test_Vision_IMS_instance_name(self):
        """Test the 'instance name' setting"""
        self.assertEqual(vision_ims_extras.Vision_IMS_instance_name(), 'Vision_IMS')

    def test_Vision_IMS_base_url(self):
        """Test that the base URL tag returns correctly"""
        self.assertEqual(vision_ims_extras.Vision_IMS_base_url(), '')

    def test_Vision_IMS_is_release(self):
        """Test that the release version check functions as expected"""
        self.assertEqual(vision_ims_extras.Vision_IMS_is_release(), not version.isVision_IMSDevelopmentVersion())

    def test_Vision_IMS_docs_version(self):
        """Test that the documentation version template tag returns correctly"""
        self.assertEqual(vision_ims_extras.Vision_IMS_docs_version(), version.Vision_IMSDocsVersion())

    def test_hash(self):
        """Test that the commit hash template tag returns correctly"""
        result_hash = vision_ims_extras.Vision_IMS_commit_hash()
    ...
    def test_date(self):
        ...
        d = vision_ims_extras.Vision_IMS_commit_date()
        if settings.DOCKER:  # pragma: no cover
            # Testing inside docker environment *may* return an empty git commit hash
            # In such a case, skip this check
            pass
    ...
    def test_github(self):
        ...
        self.assertIn('github.com', vision_ims_extras.Vision_IMS_github_url())

    def test_docs(self):
        """Test that the documentation URL template tag returns correctly"""
        self.assertIn('docs.Vision_IMS.org', vision_ims_extras.Vision_IMS_docs_url())

    def test_keyvalue(self):
        """Test keyvalue template tag"""
        self.assertEqual(vision_ims_extras.keyvalue({'a': 'a'}, 'a'), 'a')

    def test_mail_configured(self):
        """Test that mail configuration returns False"""
        self.assertEqual(vision_ims_extras.mail_configured(), False)

    def test_user_settings(self):
        """Test user settings"""
        result = vision_ims_extras.user_settings(self.user)
        self.assertEqual(len(result), len(Vision_IMSUserSetting.SETTINGS))

    def test_global_settings(self):
        """Test global settings"""
        result = vision_ims_extras.global_settings()
        self.assertEqual(len(result), len(Vision_IMSSetting.SETTINGS))

    def test_visible_global_settings(self):
        """Test that hidden global settings are actually hidden"""
        result = vision_ims_extras.visible_global_settings()
        ...
        for val in Vision_IMSSetting.SETTINGS.values():
...
class PartTest(TestCase):
    ...
    def test_barcode(self):
        ...
        self.assertIn('Vision_IMS', barcode)
...
class PartSettingsTest(Vision_IMSTestCase):
    ...
    def test_custom(self):
        ...
        for val in [True, False]:
            Vision_IMSSetting.set_setting('PART_COMPONENT', val, self.user)
            Vision_IMSSetting.set_setting('PART_PURCHASEABLE', val, self.user)
            Vision_IMSSetting.set_setting('PART_SALABLE', val, self.user)
            Vision_IMSSetting.set_setting('PART_TRACKABLE', val, self.user)
            Vision_IMSSetting.set_setting('PART_ASSEMBLY', val, self.user)
            Vision_IMSSetting.set_setting('PART_TEMPLATE', val, self.user)

            self.assertEqual(val, Vision_IMSSetting.get_setting('PART_COMPONENT'))
            self.assertEqual(val, Vision_IMSSetting.get_setting('PART_PURCHASEABLE'))
            self.assertEqual(val, Vision_IMSSetting.get_setting('PART_SALABLE'))
            self.assertEqual(val, Vision_IMSSetting.get_setting('PART_TRACKABLE'))
    ...
    def test_duplicate_ipn(self):
        ...
        Vision_IMSSetting.set_setting('PART_ALLOW_DUPLICATE_IPN', False, self.user)
...
class PartSubscriptionTests(Vision_IMSTestCase):
...
class BaseNotificationIntegrationTest(Vision_IMSTestCase):
