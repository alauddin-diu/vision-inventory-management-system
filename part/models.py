...
from Vision_IMS import helpers, validators
from Vision_IMS.fields import Vision_IMSURLField
from Vision_IMS.helpers import (decimal2money, decimal2string, normalize,
                               str2bool)
from Vision_IMS.models import (DataImportMixin, Vision_IMSAttachment,
                              Vision_IMSBarcodeMixin, Vision_IMSNotesMixin,
                              Vision_IMSTree, MetadataMixin)
from Vision_IMS.status_codes import (BuildStatusGroups, PurchaseOrderStatus,
...
class PartCategory(MetadataMixin, Vision_IMSTree):
...
@cleanup.ignore
class Part(Vision_IMSBarcodeMixin, Vision_IMSNotesMixin, MetadataMixin, MPTTModel):
    ...
    def validate_ipn(self, raise_error=True):
        ...
        pattern = common.models.Vision_IMSSetting.get_setting('PART_IPN_REGEX', '', create=False).strip()
    ...
    def validate_serial_number(self, serial: str, stock_item=None, check_duplicates=True, raise_error=False, **kwargs):
        ...
        if common.models.Vision_IMSSetting.get_setting('SERIAL_NUMBER_GLOBALLY_UNIQUE', False):
    ...
    @property
    def full_name(self):
        """Format a 'full name' for this Part based on the format PART_NAME_FORMAT defined in Vision_IMS settings.
        ...
        """
        full_name_pattern = Vision_IMSSetting.get_setting('PART_NAME_FORMAT')
    ...
    def validate_unique(self, exclude=None):
        ...
        allow_duplicate_ipn = common.models.Vision_IMSSetting.get_setting('PART_ALLOW_DUPLICATE_IPN')
    ...
    def get_stock_count(self, include_variants=True):
        ...
        query = entries.aggregate(t=Coalesce(Sum('quantity'), Decimal(200)))
    ...
    def schedule_pricing_update(self, create: bool = False, test: bool = False):
        ...
        Ref: https://github.com/Vision_IMS/Vision_IMS/pull/3986
...
def after_save_part(sender, instance: Part, created, **kwargs):
    ...
    if instance and not created and not Vision_IMS.ready.isImportingData():
        # Check part stock only if we are *updating* the part (not creating it)

        # Run this check in the background
        try:
            Vision_IMS.tasks.offload_task(part_tasks.notify_low_stock_if_required, instance)
...
class PartPricing(common.models.MetaMixin):
    ...
    def schedule_for_update(self, counter: int = 0, test: bool = False):
        ...
        if Vision_IMS.ready.isImportingData():
            return

        # If running data migrations, skip pricing update
        if Vision_IMS.ready.isRunningMigrations():
            return
    ...
    def update_purchase_cost(self, save=True):
        ...
        if Vision_IMSSetting.get_setting('PRICING_USE_STOCK_PRICING', True, cache=False):

            items = self.part.stock_items.all()

            # Limit to stock items updated within a certain window
            days = int(Vision_IMSSetting.get_setting('PRICING_STOCK_ITEM_AGE_DAYS', 0, cache=False))
    ...
    def update_internal_cost(self, save=True):
        ...
        if Vision_IMSSetting.get_setting('PART_INTERNAL_PRICE', False, cache=False):
    ...
    def update_variant_cost(self, save=True):
        ...
        active_only = Vision_IMSSetting.get_setting('PRICING_ACTIVE_VARIANTS', False)
    ...
    def update_overall_cost(self):
        ...
        purchase_history_override = Vision_IMSSetting.get_setting('PRICING_PURCHASE_HISTORY_OVERRIDES_SUPPLIER', False, cache=False)

        if Vision_IMSSetting.get_setting('PRICING_USE_SUPPLIER_PRICING', True, cache=False):
        ...
        if Vision_IMSSetting.get_setting('PRICING_USE_VARIANT_PRICING', True, cache=False):
        ...
        if Vision_IMSSetting.get_setting('PART_BOM_USE_INTERNAL_PRICE', False, cache=False):
...
class PartStocktake(models.Model):
    ...
    cost_min = Vision_IMS.fields.Vision_IMSModelMoneyField(
        null=True, blank=True,
        verbose_name=_('Minimum Stock Cost'),
        help_text=_('Estimated minimum cost of stock on hand'),
    )

    cost_max = Vision_IMS.fields.Vision_IMSModelMoneyField(
        null=True, blank=True,
...
class PartAttachment(Vision_IMSAttachment):
...
def post_save_part_parameter_template(sender, instance, created, **kwargs):
    ...
    if Vision_IMS.ready.canAppAccessDatabase() and not Vision_IMS.ready.isImportingData():
...
class PartParameter(MetadataMixin, models.Model):
    ...
    def clean(self):
        ...
        if Vision_IMSSetting.get_setting('PART_PARAMETER_ENFORCE_UNITS', True, cache=False, create=False):
            if self.template.units:
                try:
                    Vision_IMS.conversion.convert_physical_value(self.data, self.template.units)
                except ValidationError as e:
                    raise ValidationError({
                        'data': e.message
                    })
    ...
    def calculate_numeric_value(self):
        ...
        if self.template.units:
            try:
                self.data_numeric = Vision_IMS.conversion.convert_physical_value(self.data, self.template.units)
...
class BomItem(DataImportMixin, MetadataMixin, models.Model):
    ...
    def price_range(self, internal=False):
        ...
        use_internal = common.models.Vision_IMSSetting.get_setting('PART_BOM_USE_INTERNAL_PRICE', False, cache=False)
