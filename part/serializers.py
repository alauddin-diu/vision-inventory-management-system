...
import Vision_IMS.helpers
import Vision_IMS.serializers
import Vision_IMS.status
import part.filters
import part.stocktake
import part.tasks
import stock.models
from Vision_IMS.status_codes import BuildStatusGroups
from Vision_IMS.tasks import offload_task
...
class CategorySerializer(Vision_IMS.serializers.Vision_IMSModelSerializer):
...
class CategoryTree(Vision_IMS.serializers.Vision_IMSModelSerializer):
...
class PartAttachmentSerializer(Vision_IMS.serializers.Vision_IMSAttachmentSerializer):
    ...
    class Meta:
        ...
        fields = Vision_IMS.serializers.Vision_IMSAttachmentSerializer.attachment_fields([
            'part',
        ])


class PartTestTemplateSerializer(Vision_IMS.serializers.Vision_IMSModelSerializer):
...
class PartSalePriceSerializer(Vision_IMS.serializers.Vision_IMSModelSerializer):
    ...
    quantity = Vision_IMS.serializers.Vision_IMSDecimalField()

    price = Vision_IMS.serializers.Vision_IMSMoneySerializer(allow_null=True)

    price_currency = Vision_IMS.serializers.Vision_IMSCurrencySerializer(help_text=_('Purchase currency of this stock item'))


class PartInternalPriceSerializer(Vision_IMS.serializers.Vision_IMSModelSerializer):
    ...
    price = Vision_IMS.serializers.Vision_IMSMoneySerializer(
        allow_null=True
    )
...
class PartThumbSerializerUpdate(Vision_IMS.serializers.Vision_IMSModelSerializer):
    ...
    image = Vision_IMS.serializers.Vision_IMSAttachmentSerializerField(required=True)


class PartParameterTemplateSerializer(Vision_IMS.serializers.Vision_IMSModelSerializer):
...
class PartParameterSerializer(Vision_IMS.serializers.Vision_IMSModelSerializer):
...
class PartBriefSerializer(Vision_IMS.serializers.Vision_IMSModelSerializer):
    ...
    # Pricing fields
    pricing_min = Vision_IMS.serializers.Vision_IMSMoneySerializer(source='pricing_data.overall_min', allow_null=True, read_only=True)
    pricing_max = Vision_IMS.serializers.Vision_IMSMoneySerializer(source='pricing_data.overall_max', allow_null=True, read_only=True)
...
class PartSerializer(Vision_IMS.serializers.RemoteImageMixin, Vision_IMS.serializers.Vision_IMSTagModelSerializer):
    ...
    image = Vision_IMS.serializers.Vision_IMSImageSerializerField(required=False, allow_null=True)
    thumbnail = serializers.CharField(source='get_thumbnail_url', read_only=True)
    starred = serializers.SerializerMethodField()
...
class PartStocktakeSerializer(Vision_IMS.serializers.Vision_IMSModelSerializer):
    ...
    user_detail = Vision_IMS.serializers.UserSerializer(source='user', read_only=True, many=False)

    cost_min = Vision_IMS.serializers.Vision_IMSMoneySerializer(allow_null=True)
    cost_min_currency = Vision_IMS.serializers.Vision_IMSCurrencySerializer()

    cost_max = Vision_IMS.serializers.Vision_IMSMoneySerializer(allow_null=True)
    cost_max_currency = Vision_IMS.serializers.Vision_IMSCurrencySerializer()
...
class PartStocktakeReportSerializer(Vision_IMS.serializers.Vision_IMSModelSerializer):
    ...
    report = Vision_IMS.serializers.Vision_IMSAttachmentSerializerField(read_only=True)
...
class PartStocktakeReportGenerateSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if not common.models.Vision_IMSSetting.get_setting('STOCKTAKE_ENABLE', False):
            raise serializers.ValidationError(_("Stocktake functionality is not enabled"))

        # Check that background worker is running
        if not Vision_IMS.status.is_worker_running():
...
class PartPricingSerializer(Vision_IMS.serializers.Vision_IMSModelSerializer):
    ...
    # Custom serializers
    bom_cost_min = Vision_IMS.serializers.Vision_IMSMoneySerializer(allow_null=True, read_only=True)
    bom_cost_max = Vision_IMS.serializers.Vision_IMSMoneySerializer(allow_null=True, read_only=True)

    purchase_cost_min = Vision_IMS.serializers.Vision_IMSMoneySerializer(allow_null=True, read_only=True)
    purchase_cost_max = Vision_IMS.serializers.Vision_IMSMoneySerializer(allow_null=True, read_only=True)

    internal_cost_min = Vision_IMS.serializers.Vision_IMSMoneySerializer(allow_null=True, read_only=True)
    internal_cost_max = Vision_IMS.serializers.Vision_IMSMoneySerializer(allow_null=True, read_only=True)

    supplier_price_min = Vision_IMS.serializers.Vision_IMSMoneySerializer(allow_null=True, read_only=True)
    supplier_price_max = Vision_IMS.serializers.Vision_IMSMoneySerializer(allow_null=True, read_only=True)

    variant_cost_min = Vision_IMS.serializers.Vision_IMSMoneySerializer(allow_null=True, read_only=True)
    variant_cost_max = Vision_IMS.serializers.Vision_IMSMoneySerializer(allow_null=True, read_only=True)

    overall_min = Vision_IMS.serializers.Vision_IMSMoneySerializer(allow_null=True, read_only=True)
    overall_max = Vision_IMS.serializers.Vision_IMSMoneySerializer(allow_null=True, read_only=True)

    sale_price_min = Vision_IMS.serializers.Vision_IMSMoneySerializer(allow_null=True, read_only=True)
    sale_price_max = Vision_IMS.serializers.Vision_IMSMoneySerializer(allow_null=True, read_only=True)

    sale_history_min = Vision_IMS.serializers.Vision_IMSMoneySerializer(allow_null=True, read_only=True)
    sale_history_max = Vision_IMS.serializers.Vision_IMSMoneySerializer(allow_null=True, read_only=True)
    ...
    def save(self):
        ...
        if Vision_IMS.helpers.str2bool(data.get('update', False)):
            # Update part pricing
            pricing = self.instance
            pricing.update_pricing()


class PartRelationSerializer(Vision_IMS.serializers.Vision_IMSModelSerializer):
...
class PartStarSerializer(Vision_IMS.serializers.Vision_IMSModelSerializer):
...
class BomItemSubstituteSerializer(Vision_IMS.serializers.Vision_IMSModelSerializer):
...
class BomItemSerializer(Vision_IMS.serializers.Vision_IMSModelSerializer):
    ...
    quantity = Vision_IMS.serializers.Vision_IMSDecimalField(required=True)
    ...
    pricing_min = Vision_IMS.serializers.Vision_IMSMoneySerializer(source='sub_part.pricing.overall_min', allow_null=True, read_only=True)
    pricing_max = Vision_IMS.serializers.Vision_IMSMoneySerializer(source='sub_part.pricing.overall_max', allow_null=True, read_only=True)
...
class CategoryParameterTemplateSerializer(Vision_IMS.serializers.Vision_IMSModelSerializer):
...
class BomImportUploadSerializer(Vision_IMS.serializers.DataFileUploadSerializer):
...
class BomImportExtractSerializer(Vision_IMS.serializers.DataFileExtractSerializer):
