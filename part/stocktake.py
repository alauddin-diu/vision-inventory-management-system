...
def generate_stocktake_report(**kwargs):
    ...
    exclude_external = kwargs.get(
        'exclude_exernal',
        common.models.Vision_IMSSetting.get_setting('STOCKTAKE_EXCLUDE_EXTERNAL', False)
    )
    ...
    for p in parts:
        ...
        dataset.append([
            ...
            Vision_IMS.helpers.normalize(stocktake.location_cost_min.amount),
            Vision_IMS.helpers.normalize(stocktake.location_cost_max.amount),
        ])
    ...
    filename = f"Vision_IMS_Stocktake_{today}.csv"
