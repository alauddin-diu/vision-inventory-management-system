...
from Vision_IMS.tasks import (ScheduledTask, check_daily_holdoff,
...
def notify_low_stock(part: part.models.Part):
    ...
    context = {
        ...
        'name': name,
        'message': message,
        'link': Vision_IMS.helpers_model.construct_absolute_url(part.get_absolute_url()),
        'template': {
...
def check_missing_pricing(limit=250):
    ...
    days = int(common.models.Vision_IMSSetting.get_setting('PRICING_UPDATE_DAYS', 30))
...
def scheduled_stocktake_reports():
    ...
    delete_n_days = int(common.models.Vision_IMSSetting.get_setting('STOCKTAKE_DELETE_REPORT_DAYS', 30, cache=False))
    ...
    if not common.models.Vision_IMSSetting.get_setting('STOCKTAKE_ENABLE', False, cache=False):
        logger.info("Stocktake functionality is not enabled - exiting")
        return

    report_n_days = int(common.models.Vision_IMSSetting.get_setting('STOCKTAKE_AUTO_DAYS', 0, cache=False))
