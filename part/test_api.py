...
from Vision_IMS.status_codes import (BuildStatus, PurchaseOrderStatusGroups,
                                    StockStatus)
...
class PartCategoryAPITest(Vision_IMSAPITestCase):
...
class PartOptionsAPITest(Vision_IMSAPITestCase):
...
class PartAPITestBase(Vision_IMSAPITestCase):
...
class PartAPITest(PartAPITestBase):
    ...
    def test_part_download(self):
        ...
        with self.download_file(
            ...
            expected_fn='Vision_IMS_Parts.csv',
        ) as file:
...
class PartNotesTests(Vision_IMSAPITestCase):
...
class PartPricingDetailTests(Vision_IMSAPITestCase):
...
class PartAPIAggregationTest(Vision_IMSAPITestCase):
    ...
    def test_allocation_annotations(self):
        ...
        Ref: https://github.com/Vision_IMS/Vision_IMS/pull/2797
        """
...
class BomItemTest(Vision_IMSAPITestCase):
...
class PartAttachmentTest(Vision_IMSAPITestCase):
...
class PartInternalPriceBreakTest(Vision_IMSAPITestCase):
...
class PartStocktakeTest(Vision_IMSAPITestCase):
    ...
    def test_report_generate(self):
        ...
        Vision_IMSSetting.set_setting('STOCKTAKE_ENABLE', False, None)
        self.assignRole('stocktake.add')
        response = self.post(url, data={}, expected_code=400)

        self.assertIn('Stocktake functionality is not enabled', str(response.data))

        Vision_IMSSetting.set_setting('STOCKTAKE_ENABLE', True, None)
        response = self.post(url, data={}, expected_code=400)
        self.assertIn('Background worker check failed', str(response.data))


class PartMetadataAPITest(Vision_IMSAPITestCase):
