...
def check_system_health(**kwargs):
    """Check that the Vision_IMS system is running OK.

    Returns True if all system checks pass.
    """
    result = True

    if Vision_IMS.ready.isInTestMode():
    ...
    if not Vision_IMS.email.is_email_configured():  # pragma: no cover
        result = False
        logger.warning(_("Email backend not configured"))

    if not result:  # pragma: no cover
        logger.warning(_("Vision_IMS system health checks failed"))
