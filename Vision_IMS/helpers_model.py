"""Provides helper functions used throughout the Vision_IMS project that access the database."""
...
import Vision_IMS.version
from common.notifications import (Vision_IMSNotificationBodies,
...
def getSetting(key, backup_value=None):
    ...
    return common.models.Vision_IMSSetting.get_setting(key, backup_value=backup_value)
...
def construct_absolute_url(*arg, **kwargs):
    ...
    if not site_url:
        # Otherwise, try to use the Vision_IMS setting
        try:
            site_url = common.models.Vision_IMSSetting.get_setting('Vision_IMS_BASE_URL', create=False, cache=False)
...
def get_base_url(**kwargs):
    """Return the base URL for the Vision_IMS server"""
...
def download_image_from_url(remote_url, timeout=2.5):
    ...
    max_size = int(common.models.Vision_IMSSetting.get_setting('Vision_IMS_DOWNLOAD_IMAGE_MAX_SIZE')) * 1024 * 1024

    # Add user specified user-agent to request (if specified)
    user_agent = common.models.Vision_IMSSetting.get_setting('Vision_IMS_DOWNLOAD_FROM_URL_USER_AGENT')
    if user_agent:
...
def render_currency(money, decimal_places=None, currency=None, include_symbol=True, min_decimal_places=None, max_decimal_places=None):
    ...
    if decimal_places is None:
        decimal_places = common.models.Vision_IMSSetting.get_setting('PRICING_DECIMAL_PLACES', 6)

    if min_decimal_places is None:
        min_decimal_places = common.models.Vision_IMSSetting.get_setting('PRICING_DECIMAL_PLACES_MIN', 0)

    if max_decimal_places is None:
        max_decimal_places = common.models.Vision_IMSSetting.get_setting('PRICING_DECIMAL_PLACES', 6)
...
def notify_responsible(instance, sender, content: NotificationBody = Vision_IMSNotificationBodies.NewOrder, exclude=None):
    ...
    Args:
        ...
        content (NotificationBody, optional): _description_. Defaults to Vision_IMSNotificationBodies.NewOrder.
        exclude (User, optional): User instance that should be excluded. Defaults to None.
    """
    notify_users([instance.responsible], instance, sender, content=content, exclude=exclude)


def notify_users(users, instance, sender, content: NotificationBody = Vision_IMSNotificationBodies.NewOrder, exclude=None):
    ...
    context = {
        ...
        'link': Vision_IMS.helpers_model.construct_absolute_url(instance.get_absolute_url()),
        'template': {
