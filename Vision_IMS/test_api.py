"""Low level tests for the Vision_IMS API."""
...
class HTMLAPITests(Vision_IMSTestCase):
    """Test that we can access the REST API endpoints via the HTML interface.

    History: Discovered on 2021-06-28 a bug in Vision_IMSModelSerializer,
...
class APITests(Vision_IMSAPITestCase):
    """Tests for the Vision_IMS API."""
    ...
    def test_info_view(self):
        ...
        url = reverse('api-Vision_IMS-info')
        ...
        self.assertEqual('Vision_IMS', data['server'])
...
class BulkDeleteTests(Vision_IMSAPITestCase):
...
class SearchTests(Vision_IMSAPITestCase):
