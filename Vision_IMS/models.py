...
class ReferenceIndexingMixin(models.Model):
    ...
    def get_reference_pattern(cls):
        ...
        return Vision_IMSSetting.get_setting(cls.REFERENCE_PATTERN_SETTING, create=False).strip()
    ...
    def get_next_reference(cls):
        ...
        try:
            reference = Vision_IMS.format.extract_named_group('ref', reference, cls.get_reference_pattern())
        except Exception:
        ...
        incremented = Vision_IMS.helpers.increment(reference)
            ...
            try:
                ...
                if ref in attempts:
                    ...
                    if cls.objects.filter(reference=ref).exists():
                        ...
                        ctx['ref'] = Vision_IMS.helpers.increment(ctx['ref'])
                    else:
    ...
    def validate_reference_pattern(cls, pattern):
        ...
        try:
            info = Vision_IMS.format.parse_format_string(pattern)
    ...
    def validate_reference_field(cls, value):
        ...
        if not Vision_IMS.format.validate_string(value, pattern):
...
class Vision_IMSAttachment(models.Model):
    ...
    link = Vision_IMS.fields.Vision_IMSURLField(
        blank=True, null=True,
        verbose_name=_('Link'),
    ...
    def fully_qualified_url(self):
        ...
        if self.attachment:
            media_url = Vision_IMS.helpers.getMediaUrl(self.attachment.url)
            return Vision_IMS.helpers_model.construct_absolute_url(media_url)

        return ''


class Vision_IMSTree(MPTTModel):
    ...
    def api_instance_filters(self):
        """Instance filters for Vision_IMSTree models."""
        return {
            'parent': {
    ...
    def save(self, *args, **kwargs):
        """Custom save method for Vision_IMSTree abstract model"""
        ...
        pathstring = Vision_IMS.helpers.constructPathString(
...
class Vision_IMSNotesMixin(models.Model):
    ...
    notes = Vision_IMS.fields.Vision_IMSNotesField(
        verbose_name=_('Notes'),
        help_text=_('Markdown notes (optional)'),
    )


class Vision_IMSBarcodeMixin(models.Model):
    ...
    def format_barcode(self, **kwargs):
        ...
        return Vision_IMS.helpers.MakeBarcode(
    ...
    def assign_barcode(self, barcode_hash=None, barcode_data=None, raise_error=True, save=True):
        ...
        if barcode_hash is None:
            barcode_hash = Vision_IMS.helpers.hash_barcode(barcode_data)
...
@receiver(pre_delete, sender=Vision_IMSTree, dispatch_uid='tree_pre_delete_log')
def before_delete_tree_item(sender, instance, using, **kwargs):
    """Receives pre_delete signal from Vision_IMSTree object.
...
def after_error_logged(sender, instance: Error, created: bool, **kwargs):
    ...
    if created:
        try:
            ...
            link = Vision_IMS.helpers_model.construct_absolute_url(
            ...
            for user in users:
                if common.models.Vision_IMSUserSetting.get_setting('NOTIFICATION_ERROR_REPORT', True, user=user):
                    target_users.append(user)

            if len(target_users) > 0:
                common.notifications.trigger_notification(
                    instance,
                    'Vision_IMS.error_log',
                    context=context,
                    targets=users,
