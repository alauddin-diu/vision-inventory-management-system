...
from Vision_IMS.version import Vision_IMS_SW_VERSION

logger = logging.getLogger('Vision_IMS')


def default_sentry_dsn():
    """Return the default Sentry.io DSN for Vision_IMS"""
...
def init_sentry(dsn, sample_rate, tags):
    ...
    sentry_sdk.init(
        ...
        release=Vision_IMS_SW_VERSION,
    )

    for key, val in tags.items():
        sentry_sdk.set_tag(f'Vision_IMS_{key}', val)
