"""Provides helper functions used throughout the Vision_IMS project."""
...
def getLogoImage(as_file=False, custom=True):
    """Return the Vision_IMS logo image, or a custom logo if available."""
    ...
    if as_file:
        path = settings.STATIC_ROOT.joinpath('img/Vision_IMS.png')
        return f"file://{path}"
    else:
        return getStaticUrl('img/Vision_IMS.png')


def getSplashScren(custom=True):
    """Return the Vision_IMS splash screen, or a custom splash if available"""
    ...
    return static_storage.url("img/vision_ims2.jpg")
...
def MakeBarcode(cls_name, object_pk: int, object_data=None, **kwargs):
    """Generate a string for a barcode. Adds some global Vision_IMS parameters.
    ...
    if brief:
        ...
        data['tool'] = 'Vision_IMS'
        data['version'] = Vision_IMS.version.Vision_IMSVersion()
        data['instance'] = Vision_IMS.version.Vision_IMSInstanceName()
