...
def check_daily_holdoff(task_name: str, n_days: int = 1) -> bool:
    ...
    Arguments:
        ...
        setting_name: The name of the global setting, e.g. 'Vision_IMS_DUMMY_TASK_INTERVAL'
    ...
    from Vision_IMS.ready import isInTestMode
    ...
    last_success = Vision_IMSSetting.get_setting(success_key, '', cache=False)
    ...
    last_attempt = Vision_IMSSetting.get_setting(attempt_key, '', cache=False)
...
def record_task_attempt(task_name: str):
    ...
    Vision_IMSSetting.set_setting(f'_{task_name}_ATTEMPT', datetime.now().isoformat(), None)
...
def record_task_success(task_name: str):
    ...
    Vision_IMSSetting.set_setting(f'_{task_name}_SUCCESS', datetime.now().isoformat(), None)
...
def offload_task(taskname, *args, force_async=False, force_sync=False, **kwargs):
    ...
    try:
        ...
        from Vision_IMS.status import is_worker_running
    except AppRegistryNotReady:  # pragma: no cover
...
def heartbeat():
    ...
    heartbeats = Success.objects.filter(
        func='Vision_IMS.tasks.heartbeat',
...
def delete_successful_tasks():
    ...
    try:
        ...
        days = Vision_IMSSetting.get_setting('Vision_IMS_DELETE_TASKS_DAYS', 30)
...
def delete_old_error_logs():
    ...
    try:
        ...
        days = Vision_IMSSetting.get_setting('Vision_IMS_DELETE_ERRORS_DAYS', 30)
        threshold = timezone.now() - timedelta(days=days)
...
def delete_old_notifications():
    ...
    try:
        from common.models import (Vision_IMSSetting, NotificationEntry,
                                   NotificationMessage)

        days = Vision_IMSSetting.get_setting('Vision_IMS_DELETE_NOTIFICATIONS_DAYS', 30)
        threshold = timezone.now() - timedelta(days=days)
...
def check_for_updates():
    """Check if there is an update for Vision_IMS."""
    try:
    ...
    interval = int(common.models.Vision_IMSSetting.get_setting('Vision_IMS_UPDATE_CHECK_INTERVAL', 7, cache=False))

    # Check if we should check for updates *today*
    if not check_daily_holdoff('check_for_updates', interval):
        return

    logger.info("Checking for Vision_IMS software updates")
    ...
    response = requests.get(
        'https://api.github.com/repos/Vision_IMS/Vision_IMS/releases/latest',
        headers=headers
    )
    ...
    logger.info("Latest Vision_IMS version: '%s'", tag)

    # Save the version to the database
    common.models.Vision_IMSSetting.set_setting(
        '_Vision_IMS_LATEST_VERSION',
        tag,
        None
    )
...
def update_exchange_rates(force: bool = False):
    ...
    try:
        ...
        from Vision_IMS.exchange import Vision_IMSExchange
    except AppRegistryNotReady:  # pragma: no cover
        # Apps not yet loaded!
    ...
    try:
        ...
        Rate.objects.filter(backend="Vision_IMSExchange").exclude(currency__in=currency_codes()).delete()
...
def run_backup():
    ...
    if not Vision_IMSSetting.get_setting('Vision_IMS_BACKUP_ENABLE', False, cache=False):
        # Backups are not enabled - exit early
        return

    interval = int(Vision_IMSSetting.get_setting('Vision_IMS_BACKUP_DAYS', 1, cache=False))
...
def check_for_migrations():
    ...
    def set_pending_migrations(n: int):
        ...
        Vision_IMSSetting.set_setting('_PENDING_MIGRATIONS', n, None)
    ...
    if not get_setting('Vision_IMS_AUTO_UPDATE', 'auto_update'):
