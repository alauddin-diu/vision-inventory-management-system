"""Custom fields used in Vision_IMS."""
...
class Vision_IMSRestURLField(RestURLField):
...
class Vision_IMSURLField(models.URLField):
    """Custom URL field which has custom scheme validators."""

    default_validators = [AllowedURLValidator()]

    def __init__(self, **kwargs):
        """Initialization method for Vision_IMSURLField"""

        # Max length for Vision_IMSURLField is set to 200
        kwargs['max_length'] = 200
        super().__init__(**kwargs)
...
class Vision_IMSModelMoneyField(ModelMoneyField):
    ...
    def formfield(self, **kwargs):
        ...
        kwargs['form_class'] = Vision_IMSMoneyField
...
class Vision_IMSMoneyField(MoneyField):
    """Custom MoneyField for clean migrations while using dynamic currency settings."""
...
def round_decimal(value, places, normalize=False):
    ...
    if type(value) in [Decimal, float]:
        ...
        if normalize:
            ...
            value = Vision_IMS.helpers.normalize(value)
...
class Vision_IMSNotesField(models.TextField):
