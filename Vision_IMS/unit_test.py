...
class ExchangeRateMixin:
    ...
    def generate_exchange_rates(self):
        ...
        ExchangeBackend.objects.create(
            name='Vision_IMSExchange',
            base_currency='USD',
        )

        backend = ExchangeBackend.objects.get(name='Vision_IMSExchange')
...
class Vision_IMSTestCase(ExchangeRateMixin, UserMixin, TestCase):
    """Testcase with user setup buildin."""
    pass


class Vision_IMSAPITestCase(ExchangeRateMixin, UserMixin, APITestCase):
    """Base class for running Vision_IMS API tests."""
