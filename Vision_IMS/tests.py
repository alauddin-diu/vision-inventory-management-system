...
import Vision_IMS.format
import Vision_IMS.helpers
import Vision_IMS.helpers_model
import Vision_IMS.tasks
from common.models import CustomUnit, Vision_IMSSetting
from common.settings import currency_codes
from Vision_IMS.sanitizer import sanitize_svg
...
class ConversionTest(TestCase):
    ...
    def test_prefixes(self):
        ...
        for val, expected in tests.items():
            q = Vision_IMS.conversion.convert_physical_value(val, 'm')
    ...
    def test_base_units(self):
        ...
        for val, expected in tests.items():
            q = Vision_IMS.conversion.convert_physical_value(val, 'W')
            self.assertAlmostEqual(q, expected, places=2)
            q = Vision_IMS.conversion.convert_physical_value(val, 'W', strip_units=False)
    ...
    def test_dimensionless_units(self):
        ...
        for val, expected in tests.items():
            ...
            q = Vision_IMS.conversion.convert_physical_value(val, strip_units=False)
            self.assertAlmostEqual(float(q.magnitude), expected, 3)

            # Convert, and strip units
            q = Vision_IMS.conversion.convert_physical_value(val)
            self.assertAlmostEqual(q, expected, 3)
    ...
    def test_invalid_values(self):
        ...
        for val in inputs:
            ...
            with self.assertRaises(ValidationError):
                Vision_IMS.conversion.convert_physical_value(val, 'meter')

            # Test dimensionless
            with self.assertRaises(ValidationError):
                Vision_IMS.conversion.convert_physical_value(val)
    ...
    def test_custom_units(self):
        ...
        Vision_IMS.conversion.reload_unit_registry()

        # Ensure that the custom unit does *not* exist to start with
        reg = Vision_IMS.conversion.get_unit_registry()
        ...
        for val, expected in tests.items():
            # Convert, and leave units
            q = Vision_IMS.conversion.convert_physical_value(val, 'henry / km', strip_units=False)
            self.assertAlmostEqual(float(q.magnitude), expected, 2)

            # Convert and strip units
            q = Vision_IMS.conversion.convert_physical_value(val, 'henry / km')
...
class FormatTest(TestCase):
    ...
    def test_parse(self):
        ...
        info = Vision_IMS.format.parse_format_string(fmt)
        ...
        for fmt in [
            ...
            with self.assertRaises(ValueError):
                Vision_IMS.format.parse_format_string(fmt)
    ...
    def test_create_regex(self):
        ...
        for fmt, reg in tests.items():
            self.assertEqual(Vision_IMS.format.construct_format_regex(fmt), reg)
    ...
    def test_validate_format(self):
        ...
        for value, pattern in {
            ...
            self.assertTrue(Vision_IMS.format.validate_string(value, pattern))
        ...
        for value, pattern in {
            ...
            self.assertFalse(Vision_IMS.format.validate_string(value, pattern))
    ...
    def test_extract_value(self):
        ...
        for k, v in tests.items():
            self.assertEqual(Vision_IMS.format.extract_named_group('ref', v, fmt), k)
        ...
        for v in tests.values():
            with self.assertRaises(ValueError):
                Vision_IMS.format.extract_named_group('ref', v, fmt)
        ...
        for k, v in data.items():
            self.assertEqual(Vision_IMS.format.extract_named_group(k, val, fmt), v)

        # Test for error conditions

        # Raises a ValueError as the format string is bad
        with self.assertRaises(ValueError):
            Vision_IMS.format.extract_named_group(
                "test",
                "PO-1234-5",
                "PO-{test}-{"
            )
...
class TestHelpers(TestCase):
    """Tests for Vision_IMS helper functions."""

    def test_absolute_url(self):
        """Test helper function for generating an absolute URL"""

        base = "https://demo.Vision_IMS.org:12345"

        Vision_IMSSetting.set_setting('Vision_IMS_BASE_URL', base, change_user=None)
        ...
        tests = {
            ...
            "https://demo.Vision_IMS.org:12345/out.html": "https://demo.Vision_IMS.org:12345/out.html",
            "https://demo.Vision_IMS.org/test.html": "https://demo.Vision_IMS.org/test.html",
        ...
        for url, expected in tests.items():
            ...
            self.assertEqual(Vision_IMS.helpers_model.construct_absolute_url(url, site_url=base), expected)

            # Test without supplied base URL
            self.assertEqual(Vision_IMS.helpers_model.construct_absolute_url(url), expected)
    ...
    def test_logo_image(self):
        ...
        self.assertEqual(logo, '/static/img/Vision_IMS.png')

        logo = helpers.getLogoImage(as_file=True)
        self.assertEqual(logo, f'file://{settings.STATIC_ROOT}/img/Vision_IMS.png')
    ...
    def test_download_image(self):
        ...
        for url in [
            ...
            with self.assertRaises(django_exceptions.ValidationError):
                Vision_IMS.helpers_model.download_image_from_url(url)
        ...
        def dl_helper(url, expected_error, timeout=2.5, retries=3):
            ...
            with self.assertRaises(expected_error):
                while tries < retries:
                    ...
                    try:
                        Vision_IMS.helpers_model.download_image_from_url(url, timeout=timeout)
                        break
        ...
        large_img = "https://github.com/Vision_IMS/Vision_IMS/raw/master/Vision_IMS/Vision_IMS/static/img/paper_splash_large.jpg"

        Vision_IMSSetting.set_setting('Vision_IMS_DOWNLOAD_IMAGE_MAX_SIZE', 1, change_user=None)

        # Attempt to download an image which is too large
        with self.assertRaises(ValueError):
            Vision_IMS.helpers_model.download_image_from_url(large_img, timeout=10)

        # Increase allowable download size
        Vision_IMSSetting.set_setting('Vision_IMS_DOWNLOAD_IMAGE_MAX_SIZE', 5, change_user=None)
    ...
    def test_model_mixin(self):
        ...
        models = Vision_IMS.helpers_model.getModelsWithMixin(Vision_IMSBarcodeMixin)
        ...
        self.assertNotIn(Vision_IMSSetting, models)
...
class TestMakeBarcode(TestCase):
    ...
    def test_barcode_extended(self):
        ...
        self.assertIn('"tool": "Vision_IMS"', bc)
...
class TestVersionNumber(TestCase):
    ...
    def test_tuple(self):
        """Test Vision_IMSVersionTuple."""
        v = version.Vision_IMSVersionTuple()
        self.assertEqual(len(v), 3)

        s = '.'.join([str(i) for i in v])

        self.assertTrue(s in version.Vision_IMSVersion())

    def test_comparison(self):
        """Test direct comparison of version numbers."""
        v_a = version.Vision_IMSVersionTuple('1.2.0')
        v_b = version.Vision_IMSVersionTuple('1.2.3')
        v_c = version.Vision_IMSVersionTuple('1.2.4')
        v_d = version.Vision_IMSVersionTuple('2.0.0')
    ...
    def test_commit_info(self):
        ...
        envs = {
            'Vision_IMS_COMMIT_HASH': 'abcdef',
            'Vision_IMS_COMMIT_DATE': '2022-12-31'
        }

        # Check that the environment variables take priority

        with mock.patch.dict(os.environ, envs):
            self.assertEqual(version.Vision_IMSCommitHash(), 'abcdef')
            self.assertEqual(version.Vision_IMSCommitDate(), '2022-12-31')
        ...
        self.assertEqual(hash, version.Vision_IMSCommitHash())

        d = str(subprocess.check_output('git show -s --format=%ci'.split()), 'utf-8').strip().split(' ')[0]
        self.assertEqual(d, version.Vision_IMSCommitDate())
...
class CurrencyTests(TestCase):
    ...
    def test_rates(self):
        ...
        for _ in range(10):
            Vision_IMS.tasks.update_exchange_rates()
...
class TestSettings(Vision_IMSTestCase):
    ...
    def run_reload(self, envs=None):
        """Helper function to reload Vision_IMS."""
        # Set default - see B006
        if envs is None:
            envs = {}
    ...
    def test_set_user_to_few(self):
        ...
        # not enough set
        self.run_reload({
            'Vision_IMS_ADMIN_USER': 'admin'
        })
        self.assertEqual(user_count(), 1)

        # enough set
        self.run_reload({
            'Vision_IMS_ADMIN_USER': 'admin',  # set username
            'Vision_IMS_ADMIN_EMAIL': 'info@example.com',  # set email
            'Vision_IMS_ADMIN_PASSWORD': 'password123'  # set password
        })
        ...
        self.run_reload({
            'Vision_IMS_ADMIN_USER': username2,
            'Vision_IMS_ADMIN_EMAIL': email2,
            'Vision_IMS_ADMIN_PASSWORD': password2,
        })
        self.assertEqual(user_count(), 3)
    ...
    def test_initial_install(self):
        ...
        if not settings.DOCKER:
            ...
            Vision_IMSSetting.set_setting('PLUGIN_ON_STARTUP', True, self.user)
    ...
    def test_helpers_cfg_file(self):
        ...
        valid = [
            'Vision_IMS/config.yaml',
            'Vision_IMS/data/config.yaml',
        ]

        self.assertTrue(any(opt in str(config.get_config_file()).lower() for opt in valid))

        # with env set
        with self.in_env_context({'Vision_IMS_CONFIG_FILE': 'my_special_conf.yaml'}):
            self.assertIn('Vision_IMS/my_special_conf.yaml', str(config.get_config_file()).lower())
    ...
    def test_helpers_plugin_file(self):
        ...
        valid = [
            'Vision_IMS/plugins.txt',
            'Vision_IMS/data/plugins.txt',
        ]

        self.assertTrue(any(opt in str(config.get_plugin_file()).lower() for opt in valid))

        # with env set
        with self.in_env_context({'Vision_IMS_PLUGIN_FILE': 'my_special_plugins.txt'}):
...
class TestInstanceName(Vision_IMSTestCase):
    """Unit tests for instance name."""

    def test_instance_name(self):
        """Test instance name settings."""
        # default setting
        self.assertEqual(version.Vision_IMSInstanceTitle(), 'Vision_IMS')

        # set up required setting
        Vision_IMSSetting.set_setting("Vision_IMS_INSTANCE_TITLE", True, self.user)
        Vision_IMSSetting.set_setting("Vision_IMS_INSTANCE", "Testing title", self.user)

        self.assertEqual(version.Vision_IMSInstanceTitle(), 'Testing title')
    ...
    def test_instance_url(self):
        ...
        # Set up required setting
        Vision_IMSSetting.set_setting("Vision_IMS_BASE_URL", "http://127.1.2.3", self.user)
...
class TestOffloadTask(Vision_IMSTestCase):
    ...
    def test_offload_tasks(self):
        ...
        Ref: https://github.com/Vision_IMS/Vision_IMS/pull/3273
        """
    ...
    def test_daily_holdoff(self):
        ...
        with self.assertLogs(logger='Vision_IMS', level='INFO') as cm:
            # With a non-positive interval, task will not run
            result = Vision_IMS.tasks.check_daily_holdoff('some_task', 0)
            self.assertFalse(result)
            self.assertIn('Specified interval', str(cm.output))

        with self.assertLogs(logger='Vision_IMS', level='INFO') as cm:
            # First call should run without issue
            result = Vision_IMS.tasks.check_daily_holdoff('dummy_task')
        ...
        Vision_IMSSetting.set_setting('_dummy_task_ATTEMPT', t_old, None)

        result = Vision_IMS.tasks.check_daily_holdoff('dummy_task', 5)
        self.assertTrue(result)

        # Last attempt should have been updated
        self.assertNotEqual(t_old, Vision_IMSSetting.get_setting('_dummy_task_ATTEMPT', '', cache=False))
        ...
        Vision_IMSSetting.set_setting('_dummy_task_ATTEMPT', t_old, None)
        Vision_IMSSetting.set_setting('_dummy_task_SUCCESS', t_old, None)

        with self.assertLogs(logger='Vision_IMS', level='INFO') as cm:
            result = Vision_IMS.tasks.check_daily_holdoff('dummy_task', 7)
            self.assertFalse(result)
            self.assertIn('Last successful run for', str(cm.output))

            result = Vision_IMS.tasks.check_daily_holdoff('dummy_task', 2)
            self.assertTrue(result)


class BarcodeMixinTest(Vision_IMSTestCase):
    """Tests for the Vision_IMSBarcodeMixin mixin class"""
    ...
    def test_barcode_hash(self):
        ...
        for barcode, hash in hashing_tests.items():
            self.assertEqual(Vision_IMS.helpers.hash_barcode(barcode), hash)
...
class MagicLoginTest(Vision_IMSTestCase):
