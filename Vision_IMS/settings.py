"""Django settings for Vision_IMS project.
...
from Vision_IMS.config import get_boolean_setting, get_custom_file, get_setting
from Vision_IMS.sentry import default_sentry_dsn, init_sentry
from Vision_IMS.version import Vision_IMSApiVersion

from . import config

Vision_IMS_NEWS_URL = 'https://Vision_IMS.org/news/feed.atom'
...
if TESTING:
    ...
    if os.getenv('Vision_IMS_DOCKER'):
...
DEBUG = get_boolean_setting('Vision_IMS_DEBUG', 'debug', True)

ENABLE_CLASSIC_FRONTEND = get_boolean_setting('Vision_IMS_CLASSIC_FRONTEND', 'classic_frontend', True)
ENABLE_PLATFORM_FRONTEND = get_boolean_setting('Vision_IMS_PLATFORM_FRONTEND', 'platform_frontend', True)

# Configure logging settings
log_level = get_setting('Vision_IMS_LOG_LEVEL', 'log_level', 'WARNING')
...
if get_setting('Vision_IMS_DB_LOGGING', 'db_logging', False):
    LOGGING['loggers'] = {
        'django.db.backends': {
...
STATIC_ROOT = '/Vision_IMS/Vision_IMS/static'
...
CORS_ORIGIN_ALLOW_ALL = get_boolean_setting(
    "Vision_IMS_CORS_ORIGIN_ALLOW_ALL",
    config_key='cors.allow_all',
    default_value=False,
)

CORS_ORIGIN_WHITELIST = get_setting(
    "Vision_IMS_CORS_ORIGIN_WHITELIST",
...
STATICFILES_I18_TRG = BASE_DIR.joinpath('Vision_IMS', 'static_i18n')
STATICFILES_DIRS.append(STATICFILES_I18_TRG)
STATICFILES_I18_TRG = STATICFILES_I18_TRG.joinpath(STATICFILES_I18_PREFIX)

STATFILES_I18_PROCESSORS = [
    'Vision_IMS.context.status_codes',
]

# Color Themes Directory
STATIC_COLOR_THEMES_DIR = 'Vision_IMS/Vision_IMS/static/css/color-themes'
...
DBBACKUP_STORAGE = get_setting(
    'Vision_IMS_BACKUP_STORAGE',
    'backup_storage',
    'django.core.files.storage.FileSystemStorage'
)

# Default backup configuration
DBBACKUP_STORAGE_OPTIONS = get_setting('Vision_IMS_BACKUP_OPTIONS', 'backup_options', None)
if DBBACKUP_STORAGE_OPTIONS is None:
    DBBACKUP_STORAGE_OPTIONS = {
        'location': 'Vision_IMS/backup',
    }
...
INSTALLED_APPS = [
    ...
    'web',
    'generic',
    'Vision_IMS.apps.Vision_IMSConfig',       # Vision_IMS app runs last
...
MIDDLEWARE = CONFIG.get('middleware', [
    ...
    'Vision_IMS.middleware.Vision_IMSRemoteUserMiddleware',       # Remote / proxy auth
    'django_otp.middleware.OTPMiddleware',                      # MFA support
    'Vision_IMS.middleware.CustomAllauthTwoFactorMiddleware',    # Flow control for allauth
    'django.contrib.messages.middleware.MessageMiddleware',
    'django.middleware.clickjacking.XFrameOptionsMiddleware',
    'Vision_IMS.middleware.AuthRequiredMiddleware',
    'Vision_IMS.middleware.Check2FAMiddleware',                  # Check if the user should be forced to use MFA
    'maintenance_mode.middleware.MaintenanceModeMiddleware',
    'Vision_IMS.middleware.Vision_IMSExceptionProcessor',         # Error reporting
])
...
# LDAP support
LDAP_AUTH = get_boolean_setting("Vision_IMS_LDAP_ENABLED", "ldap.enabled", False)
if LDAP_AUTH:
    import ldap
    ...
    LDAP_DEBUG = get_boolean_setting("Vision_IMS_LDAP_DEBUG", "ldap.debug", False)
    if LDAP_DEBUG:
    ...
    global_options_dict = get_setting("Vision_IMS_LDAP_GLOBAL_OPTIONS", "ldap.global_options", {}, dict)
    global_options = {}
    ...
    AUTH_LDAP_SERVER_URI = get_setting("Vision_IMS_LDAP_SERVER_URI", "ldap.server_uri")
    AUTH_LDAP_START_TLS = get_boolean_setting("Vision_IMS_LDAP_START_TLS", "ldap.start_tls", False)
    AUTH_LDAP_BIND_DN = get_setting("Vision_IMS_LDAP_BIND_DN", "ldap.bind_dn")
    AUTH_LDAP_BIND_PASSWORD = get_setting("Vision_IMS_LDAP_BIND_PASSWORD", "ldap.bind_password")
    AUTH_LDAP_USER_SEARCH = LDAPSearch(
        get_setting("Vision_IMS_LDAP_SEARCH_BASE_DN", "ldap.search_base_dn"),
        ldap.SCOPE_SUBTREE,
        str(get_setting("Vision_IMS_LDAP_SEARCH_FILTER_STR", "ldap.search_filter_str", "(uid= %(user)s)"))
    )
    AUTH_LDAP_USER_DN_TEMPLATE = get_setting("Vision_IMS_LDAP_USER_DN_TEMPLATE", "ldap.user_dn_template")
    AUTH_LDAP_USER_ATTR_MAP = get_setting("Vision_IMS_LDAP_USER_ATTR_MAP", "ldap.user_attr_map", {
        'first_name': 'givenName',
        'last_name': 'sn',
        'email': 'mail',
    }, dict)
    AUTH_LDAP_ALWAYS_UPDATE_USER = get_boolean_setting("Vision_IMS_LDAP_ALWAYS_UPDATE_USER", "ldap.always_update_user", True)
    AUTH_LDAP_CACHE_TIMEOUT = get_setting("Vision_IMS_LDAP_CACHE_TIMEOUT", "ldap.cache_timeout", 3600, int)

DEBUG_TOOLBAR_ENABLED = DEBUG and get_setting('Vision_IMS_DEBUG_TOOLBAR', 'debug_toolbar', False)
...
# Vision_IMS URL configuration

# Base URL for admin pages (default="admin")
VISION_IMS_ADMIN_URL = get_setting(
    'Vision_IMS_ADMIN_URL',
    config_key='admin_url',
    default_value='admin'
)

ROOT_URLCONF = 'Vision_IMS.urls'
...
TEMPLATES = [
    {
        ...
        'OPTIONS': {
            'context_processors': [
                ...
                # Custom Vision_IMS context processors
                'Vision_IMS.context.health_status',
                'Vision_IMS.context.status_codes',
                'Vision_IMS.context.user_roles',
            ],
            'loaders': [(
                'Vision_IMS.template.Vision_IMSTemplateLoader', [
...
REST_FRAMEWORK = {
    'EXCEPTION_HANDLER': 'Vision_IMS.exceptions.exception_handler',
    ...
    'DEFAULT_PERMISSION_CLASSES': (
        ...
        'Vision_IMS.permissions.RolePermission',
    ),
    'DEFAULT_SCHEMA_CLASS': 'drf_spectacular.openapi.AutoSchema',
    'DEFAULT_METADATA_CLASS': 'Vision_IMS.metadata.Vision_IMSMetadata',
...
# dj-rest-auth
# JWT switch
USE_JWT = get_boolean_setting('Vision_IMS_USE_JWT', 'use_jwt', False)
REST_USE_JWT = USE_JWT
OLD_PASSWORD_FIELD_ENABLED = True
REST_AUTH_REGISTER_SERIALIZERS = {'REGISTER_SERIALIZER': 'Vision_IMS.forms.CustomRegisterSerializer'}

# JWT settings - rest_framework_simplejwt
if USE_JWT:
    JWT_AUTH_COOKIE = 'Vision_IMS-auth'
    JWT_AUTH_REFRESH_COOKIE = 'Vision_IMS-token'
...
SPECTACULAR_SETTINGS = {
    'TITLE': 'Vision_IMS API',
    'DESCRIPTION': 'API for Vision_IMS - the intuitive open source inventory management system',
    'LICENSE': {'MIT': 'https://github.com/Vision_IMS/Vision_IMS/blob/master/LICENSE'},
    'EXTERNAL_DOCS': {'docs': 'https://docs.Vision_IMS.org', 'web': 'https://Vision_IMS.org'},
    'VERSION': Vision_IMSApiVersion(),
    'SERVE_INCLUDE_SCHEMA': False,
}

WSGI_APPLICATION = 'Vision_IMS.wsgi.application'
...
if "postgres" in db_engine:  # pragma: no cover
    ...
    if "connect_timeout" not in db_options:
        ...
        db_options["connect_timeout"] = int(
            get_setting('Vision_IMS_DB_TIMEOUT', 'database.timeout', 2)
        )
    ...
    if "keepalives" not in db_options:
        db_options["keepalives"] = int(
            get_setting('Vision_IMS_DB_TCP_KEEPALIVES', 'database.tcp_keepalives', 1)
        )

    # Seconds after connection is idle to send keep alive
    if "keepalives_idle" not in db_options:
        db_options["keepalives_idle"] = int(
            get_setting('Vision_IMS_DB_TCP_KEEPALIVES_IDLE', 'database.tcp_keepalives_idle', 1)
        )
    ...
    if "keepalives_interval" not in db_options:
        db_options["keepalives_interval"] = int(
            get_setting("Vision_IMS_DB_TCP_KEEPALIVES_INTERVAL", "database.tcp_keepalives_internal", "1")
        )
    ...
    if "keepalives_count" not in db_options:
        db_options["keepalives_count"] = int(
            get_setting("Vision_IMS_DB_TCP_KEEPALIVES_COUNT", "database.tcp_keepalives_count", "5")
        )
    ...
    if "isolation_level" not in db_options:
        serializable = get_boolean_setting('Vision_IMS_DB_ISOLATION_SERIALIZABLE', 'database.serializable', False)
...
# login settings
REMOTE_LOGIN = get_boolean_setting('Vision_IMS_REMOTE_LOGIN', 'remote_login_enabled', False)
REMOTE_LOGIN_HEADER = get_setting('Vision_IMS_REMOTE_LOGIN_HEADER', 'remote_login_header', 'REMOTE_USER')

# sentry.io integration for error reporting
SENTRY_ENABLED = get_boolean_setting('Vision_IMS_SENTRY_ENABLED', 'sentry_enabled', False)

# Default Sentry DSN (can be overridden if user wants custom sentry integration)
SENTRY_DSN = get_setting('Vision_IMS_SENTRY_DSN', 'sentry_dsn', default_sentry_dsn())
SENTRY_SAMPLE_RATE = float(get_setting('Vision_IMS_SENTRY_SAMPLE_RATE', 'sentry_sample_rate', 0.1))
...
if SENTRY_ENABLED and SENTRY_DSN:  # pragma: no cover
    ...
    init_sentry(SENTRY_DSN, SENTRY_SAMPLE_RATE, Vision_IMS_tags)

# Cache configuration
cache_host = get_setting('Vision_IMS_CACHE_HOST', 'cache.host', None)
cache_port = get_setting('Vision_IMS_CACHE_PORT', 'cache.port', '6379', typecast=int)
...
_q_worker_timeout = int(get_setting('Vision_IMS_BACKGROUND_TIMEOUT', 'background.timeout', 90))

# django-q background worker configuration
Q_CLUSTER = {
    'name': 'Vision_IMS',
    'label': 'Background Tasks',
    'workers': int(get_setting('Vision_IMS_BACKGROUND_WORKERS', 'background.workers', 4)),
    'timeout': _q_worker_timeout,
    'retry': min(120, _q_worker_timeout + 30),
    'max_attempts': int(get_setting('Vision_IMS_BACKGROUND_MAX_ATTEMPTS', 'background.max_attempts', 5)),
    'queue_limit': 50,
    'catch_up': False,
    'bulk': 10,
    'orm': 'default',
...
LOGOUT_REDIRECT_URL = get_setting('Vision_IMS_LOGOUT_REDIRECT_URL', 'logout_redirect_url', 'index')
...
EXTRA_URL_SCHEMES = get_setting('Vision_IMS_EXTRA_URL_SCHEMES', 'extra_url_schemes', [])
...
LANGUAGE_CODE = get_setting('Vision_IMS_LANGUAGE', 'language', 'en-us')
...
CURRENCIES = get_setting(
    'Vision_IMS_CURRENCIES', 'currencies',
...
EXCHANGE_BACKEND = 'Vision_IMS.exchange.Vision_IMSExchange'

# Email configuration options
EMAIL_BACKEND = get_setting('Vision_IMS_EMAIL_BACKEND', 'email.backend', 'django.core.mail.backends.smtp.EmailBackend')
EMAIL_HOST = get_setting('Vision_IMS_EMAIL_HOST', 'email.host', '')
EMAIL_PORT = get_setting('Vision_IMS_EMAIL_PORT', 'email.port', 25, typecast=int)
EMAIL_HOST_USER = get_setting('Vision_IMS_EMAIL_USERNAME', 'email.username', '')
EMAIL_HOST_PASSWORD = get_setting('Vision_IMS_EMAIL_PASSWORD', 'email.password', '')
EMAIL_SUBJECT_PREFIX = get_setting('Vision_IMS_EMAIL_PREFIX', 'email.prefix', '[Vision_IMS] ')
EMAIL_USE_TLS = get_boolean_setting('Vision_IMS_EMAIL_TLS', 'email.tls', False)
EMAIL_USE_SSL = get_boolean_setting('Vision_IMS_EMAIL_SSL', 'email.ssl', False)

DEFAULT_FROM_EMAIL = get_setting('Vision_IMS_EMAIL_SENDER', 'email.sender', '')

# If "from" email not specified, default to the username
if not DEFAULT_FROM_EMAIL:
    DEFAULT_FROM_EMAIL = get_setting('Vision_IMS_EMAIL_USERNAME', 'email.username', '')
...
TIME_ZONE = get_setting('Vision_IMS_TIMEZONE', 'timezone', 'UTC')
...
SOCIAL_BACKENDS = get_setting('Vision_IMS_SOCIAL_BACKENDS', 'social_backends', [], typecast=list)

for app in SOCIAL_BACKENDS:
    INSTALLED_APPS.append(app)  # pragma: no cover

SOCIALACCOUNT_PROVIDERS = get_setting('Vision_IMS_SOCIAL_PROVIDERS', 'social_providers', None, typecast=dict)

SOCIALACCOUNT_STORE_TOKENS = True

# settings for allauth
ACCOUNT_EMAIL_CONFIRMATION_EXPIRE_DAYS = get_setting('Vision_IMS_LOGIN_CONFIRM_DAYS', 'login_confirm_days', 3, typecast=int)
ACCOUNT_LOGIN_ATTEMPTS_LIMIT = get_setting('Vision_IMS_LOGIN_ATTEMPTS', 'login_attempts', 5, typecast=int)
ACCOUNT_DEFAULT_HTTP_PROTOCOL = get_setting('Vision_IMS_LOGIN_DEFAULT_HTTP_PROTOCOL', 'login_default_protocol', 'http')
...
ACCOUNT_FORMS = {
    'login': 'Vision_IMS.forms.CustomLoginForm',
    'signup': 'Vision_IMS.forms.CustomSignupForm',
...
SOCIALACCOUNT_ADAPTER = 'Vision_IMS.forms.CustomSocialAccountAdapter'
ACCOUNT_ADAPTER = 'Vision_IMS.forms.CustomAccountAdapter'
...
# Are plugins enabled?
PLUGINS_ENABLED = get_boolean_setting('Vision_IMS_PLUGINS_ENABLED', 'plugins_enabled', False)

PLUGIN_FILE = config.get_plugin_file()

# Plugin test settings
PLUGIN_TESTING = get_setting('Vision_IMS_PLUGIN_TESTING', 'PLUGIN_TESTING', TESTING)                     # Are plugins being tested?
PLUGIN_TESTING_SETUP = get_setting('Vision_IMS_PLUGIN_TESTING_SETUP', 'PLUGIN_TESTING_SETUP', False)     # Load plugins from setup hooks in testing?
PLUGIN_TESTING_EVENTS = False                                                                           # Flag if events are tested right now
PLUGIN_RETRY = get_setting('Vision_IMS_PLUGIN_RETRY', 'PLUGIN_RETRY', 5)                                 # How often should plugin loading be tried?
PLUGIN_FILE_CHECKED = False                                                                             # Was the plugin file checked?

# Site URL can be specified statically, or via a run-time setting
SITE_URL = get_setting('Vision_IMS_SITE_URL', 'site_url', None)
...
CUSTOM_LOGO = get_custom_file('Vision_IMS_CUSTOM_LOGO', 'customize.logo', 'custom logo', lookup_media=True)
CUSTOM_SPLASH = get_custom_file('Vision_IMS_CUSTOM_SPLASH', 'customize.splash', 'custom splash')

CUSTOMIZE = get_setting('Vision_IMS_CUSTOMIZE', 'customize', {})

# Frontend settings
PUI_URL_BASE = get_setting('Vision_IMS_PUI_URL_BASE', 'pui_url_base', 'platform')
PUI_SETTINGS = get_setting("Vision_IMS_PUI_SETTINGS", "pui_settings", {})

if DEBUG:
    logger.info("Vision_IMS running with DEBUG enabled")
...
CUSTOM_FLAGS = get_setting('Vision_IMS_FLAGS', 'flags', None, typecast=dict)
if CUSTOM_FLAGS:
