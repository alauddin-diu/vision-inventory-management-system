"""Top-level URL lookup for Vision_IMS application.
...
admin.site.site_header = "Vision_IMS Admin"
...
apipatterns = [
    ...
    # Vision_IMS information endpoint
    path('', InfoView.as_view(), name='api-Vision_IMS-info'),
...
settings_urls = [
    ...
    re_path(r'^.*$', SettingsView.as_view(template_name='Vision_IMS/settings/settings.html'), name='settings'),
]
...
backendpatterns = [
    # "Dynamic" javascript files which are rendered using Vision_IMS templating.
...
urlpatterns = [
    # admin sites
    re_path(f'^{settings.VISION_IMS_ADMIN_URL}/error_log/', include('error_report.urls')),
    re_path(f'^{settings.VISION_IMS_ADMIN_URL}/', admin.site.urls, name='Vision_IMS-admin'),
]
