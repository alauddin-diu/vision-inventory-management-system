...
from Vision_IMS.urls import frontendpatterns
...
class AuthRequiredMiddleware(object):
    ...
    def __call__(self, request):
        ...
        if not request.user.is_authenticated:
            ...
            However when running an external application (e.g. the Vision_IMS app or Python library),
    ...
    def require_2fa(self, request):
        ...
        try:
            if url_matcher.resolve(request.path[1:]):
                return Vision_IMSSetting.get_setting('LOGIN_ENFORCE_MFA')
        except Resolver404:
            pass
        return False
...
class Vision_IMSRemoteUserMiddleware(PersistentRemoteUserMiddleware):
...
class Vision_IMSExceptionProcessor(ExceptionProcessor):
