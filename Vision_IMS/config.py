"""Helper functions for loading Vision_IMS configuration options."""
...
def get_base_dir() -> Path:
    """Returns the base (top-level) Vision_IMS directory."""
...
def get_config_file(create=True) -> Path:
    """Returns the path of the Vision_IMS configuration file.

    Note: It will be created it if does not already exist!
    """
    base_dir = get_base_dir()

    cfg_filename = os.getenv('Vision_IMS_CONFIG_FILE')
    ...
    if not cfg_filename.exists() and create:
        print("Vision_IMS configuration file 'config.yaml' not found - creating default file")
...
def get_setting(env_var=None, config_key=None, default_value=None, typecast=None):
    ...
    Arguments:
        env_var: Name of the environment variable e.g. 'Vision_IMS_STATIC_ROOT'
...
def get_media_dir(create=True):
    ...
    md = '/Vision_IMS/Vision_IMS/static'

    if not md:
        raise FileNotFoundError('Vision_IMS_MEDIA_ROOT not specified')
...
def get_static_dir(create=True):
    ...
    sd = get_setting('STATIC_ROOT', 'static_root')

    if not sd:
        raise FileNotFoundError('Vision_IMS_STATIC_ROOT not specified')
...
def get_backup_dir(create=True):
    ...
    bd = get_setting('Vision_IMS_BACKUP_DIR', 'backup_dir')

    if not bd:
        raise FileNotFoundError('Vision_IMS_BACKUP_DIR not specified')
...
def get_plugin_file():
    """Returns the path of the Vision_IMS plugins specification file.

    Note: It will be created if it does not already exist!
    """

    # Check if the plugin.txt file (specifying required plugins) is specified
    plugin_file = get_setting('Vision_IMS_PLUGIN_FILE', 'plugin_file')
    ...
    if not plugin_file.exists():
        ...
        plugin_file.write_text("# Vision_IMS Plugins (uses PIP framework to install)\n\n")
...
def get_plugin_dir():
    ...
    return get_setting('Vision_IMS_PLUGIN_DIR', 'plugin_dir')
...
def get_secret_key():
    ...
    A) Check for environment variable Vision_IMS_SECRET_KEY => Use raw key data
    B) Check for environment variable Vision_IMS_SECRET_KEY_FILE => Load key data from file
    C) Look for default key file "secret_key.txt"
    D) Create "secret_key.txt" if it does not exist
    """

    # Look for environment variable
    if secret_key := get_setting('Vision_IMS_SECRET_KEY', 'secret_key'):
        logger.info("SECRET_KEY loaded by Vision_IMS_SECRET_KEY")  # pragma: no cover
        return secret_key

    # Look for secret key file
    if secret_key_file := get_setting('Vision_IMS_SECRET_KEY_FILE', 'secret_key_file'):
