...
def check_build_stock(build: build.models.Build):
    ...
    for bom_item in part.get_bom_items():
        ...
        if available < required:
            ...
            lines.append({
                'link': Vision_IMS.helpers_model.construct_absolute_url(sub_part.get_absolute_url()),
                'part': sub_part,
                'in_stock': in_stock,
                'allocated': allocated,
                'available': available,
    ...
    if len(emails) > 0:
        ...
        context = {
            'link': Vision_IMS.helpers_model.construct_absolute_url(build.get_absolute_url()),
            'build': build,
            'part': build.part,
            'lines': lines,
        }
        ...
        Vision_IMS.email.send_email(subject, '', recipients, html_message=html_message)
...
@Vision_IMS.tasks.scheduled_task(Vision_IMS.tasks.ScheduledTask.DAILY)
