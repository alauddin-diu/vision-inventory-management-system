...
from Vision_IMS.status_codes import BuildStatus, StockStatus, StockHistoryCode, BuildStatusGroups
...
class Build(MPTTModel, Vision_IMS.models.Vision_IMSBarcodeMixin, Vision_IMS.models.Vision_IMSNotesMixin, Vision_IMS.models.MetadataMixin, Vision_IMS.models.ReferenceIndexingMixin):
    ...
    def getNextBuildNumber(cls):
        ...
        while 1:
            new_ref = Vision_IMS.helpers.increment(new_ref)
    ...
    def complete_build(self, user):
        ...
        context = {
            ...
            'link': Vision_IMS.helpers_model.construct_absolute_url(self.get_absolute_url()),
            'template': {
...
def after_save_build(sender, instance: Build, created: bool, **kwargs):
    ...
    if Vision_IMS.ready.isImportingData() or not Vision_IMS.ready.canAppAccessDatabase(allow_test=True):
        return
    ...
    if instance:
        ...
        if created:
            ...
            Vision_IMS.tasks.offload_task(build_tasks.check_build_stock, instance)

            # Notify the responsible users that the build order has been created
            Vision_IMS.helpers_model.notify_responsible(instance, sender, exclude=instance.issued_by)
...
class BuildOrderAttachment(Vision_IMS.models.Vision_IMSAttachment):
...
class BuildItem(Vision_IMS.models.MetadataMixin, models.Model):
    ...
    def clean(self):
        ...
        try:
            ...
            if self.quantity > self.stock_item.quantity:
                ...
                q = Vision_IMS.helpers.normalize(self.quantity)
                a = Vision_IMS.helpers.normalize(self.stock_item.quantity)
