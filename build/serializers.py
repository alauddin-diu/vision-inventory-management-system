...
from Vision_IMS.serializers import Vision_IMSModelSerializer, Vision_IMSAttachmentSerializer
from Vision_IMS.serializers import UserSerializer

import Vision_IMS.helpers
from Vision_IMS.serializers import Vision_IMSDecimalField
from Vision_IMS.status_codes import StockStatus
...
class BuildSerializer(Vision_IMSModelSerializer):
    ...
    def validate_output(self, output):
        ...
        if to_complete:
            ...
            if not build.is_output_fully_allocated(output):
                ...
                accept_incomplete = Vision_IMS.helpers.str2bool(self.context['request'].data.get('accept_incomplete_allocation', False))
...
class BuildOutputCreateSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if serial_numbers:
            ...
            try:
                self.serials = Vision_IMS.helpers.extract_serial_numbers(
                    serial_numbers,
                    quantity,
...
class BuildAllocationItemSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if quantity > q:
            ...
            q = Vision_IMS.helpers.clean_decimal(q)
...
class BuildItemSerializer(Vision_IMSModelSerializer):
...
class BuildLineSerializer(Vision_IMSModelSerializer):
...
class BuildAttachmentSerializer(Vision_IMSAttachmentSerializer):
